...
## Screen Transaction DataStore Keys:
    ...
    '#ACTIVE_WAREHOUSE' = string, `gl` field (gudang origin) for the O1 opening doc. Primary source: `tasks.first['gl']`, published by CustodyCountList O1 variant once tasks load. Fallback: when task.gl/#ACTIVE_WAREHOUSE is empty at O1 submit, CustodyCountSubmit resolves the single stock_location doc where `lt=='warehouse'` and dispatches its `lv` here (renderer-only, spec §2.1). Empty only when neither source resolves (degrade-safe; CF OnVehicleOpening is fl-null-tolerant). Read for {warehouseId} token and the native vehicle_check.gl write. Cleared on route change via ExecutorDesignateCard.clearO1State.

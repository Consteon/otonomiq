...
LedgerBuild buildLedgerEntry(
  ...
  final int? schemaVersion =
      int.tryParse((parsed['_v'] ?? '').toString().trim());
  ...
  parsed.forEach((String k, dynamic v) {
    ...
    if (k.startsWith('_')) return; // meta (_v, _k, unknown _x) never enters data
...
List<LedgerEntry> decodeLedgerOutbox(
  ...
  try {
    ...
    final String why = e is FormatException ? e.message : e.runtimeType.toString();
...
Future<void> enqueueLedgerPublish(String segment, String eventRowString) async {
  ...
  final int timeReceived =
      eventRow.isNotEmpty ? (int.tryParse(eventRow[0].toString()) ?? 0) : 0;
  final String receivingPage =
      eventRow.length > 1 ? eventRow[1].toString() : '';

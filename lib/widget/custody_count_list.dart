...
class _CustodyCountListState extends State<CustodyCountList> {
  ...
  void _subscribe() {
    ...
    final String aggregate =
        (widget.component['aggregate'] ?? '').toString().trim();
    ...
    final String source =
        (widget.component['source'] ?? '').toString().trim();
  ...
  List<_CountRow> _buildAggregateRows(
    ...
    final List<Map<String, dynamic>> tasks =
        excludeByStatus(filtered, excludeStatus);
    ...
    final String itemsField =
        (widget.component['aggregate'] ?? 'it').toString().trim();
    final String planField =
        (widget.component['planField'] ?? 'pd').toString().trim();
    ...
    final String writeCond =
        (widget.component['writeCond'] ?? '').toString().trim();
    final List<_CountRow> rows = <_CountRow>[];
    ...
    for (final String ii in agg.iiOrder) {
      ...
      if (filter != null) {
        ...
        final String categoryValue =
            filter.field == 'ic' ? detail.category : '';
        if (categoryValue != filter.value) continue;
      }
  ...
  List<_CountRow> _buildAssetCacheRows(
    ...
    final List<Map<String, dynamic>> acDocs =
        List<Map<String, dynamic>>.from(
    ...
    final String condField =
        (widget.component['condField'] ?? 'cd').toString().trim();
    final String qtyField =
        (widget.component['qtyField'] ?? 'qt').toString().trim();
    ...
    for (final Map<String, dynamic> doc in filtered) {
      ...
      final CountEntry existing = countMap.putIfAbsent(countKey,

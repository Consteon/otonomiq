...
String formatReceiptDate(int epochMs) {
  ...
  final DateTime dt =
      DateTime.fromMillisecondsSinceEpoch(epochMs + wibOffsetMs, isUtc: true);
...
ReceiptData extractReceiptData(
  ...
  if (rawDate is int) {
    ...
    dateFormatted =
        parsedMs != null ? formatReceiptDate(parsedMs) : rawDateStr;
  }
  ...
  if (rawLines is List) {
    for (final dynamic item in rawLines) {
      if (item is Map) {
        ...
        final int sub =
            rawSub is int ? rawSub : (int.tryParse(rawSub?.toString() ?? '') ?? 0);
...
class _ReceiptDocState extends State<ReceiptDoc> {
  ...
  void _initConfig() {
    ...
    _headerNameLiteral =
        (widget.component['headerName'] ?? '').toString().trim();
    _headerAddrLiteral =
        (widget.component['headerAddr'] ?? '').toString().trim();
    // autheniumDecode BEFORE any split on ◼ (U+25FC)
    _searchDecoded =
        (autheniumDecode(widget.component['search']) ?? '').toString().trim();
  ...
  void _subscribe() {
    ...
    if (!_hasLiteralHeader) {
      final String rawHeaderTable =
          (widget.component['headerTable'] ?? '').toString().trim();
      ...
      if (htp.tableDocId.isNotEmpty) {
        ...
        if (appVid.isNotEmpty) {
          subscribeToMapCollection(
              appVid, htp.tableDocId, htp.subColl, _headerCode);
        }
      }
    }
  }
  ...
  Map<String, String> get _fieldConfig => {
        ...
        'linePriceField':
            (widget.component['linePriceField'] ?? '').toString(),
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final List<Map<String, dynamic>> docs =
          List<Map<String, dynamic>>.from(mapTableContent[_notaCode] ?? const []);
      ...
      final String resolvedSearch =
          resolveDriverCurlyTokens(_searchDecoded, widget.scrName);

      final List<Map<String, dynamic>> matched =
          filterByCharCodeEquality(docs, resolvedSearch, screenTx);
      ...
      if (_hasLiteralHeader) {
        ...
        final String resolvedHeaderSearch =
            _headerSearchDecoded.replaceAll('{gl}', data.gl);
        final List<Map<String, dynamic>> headerMatched =
            filterByCharCodeEquality(headerDocs, resolvedHeaderSearch, screenTx);
        if (headerMatched.isNotEmpty) {
          final Map<String, dynamic> hDoc = headerMatched.first;
          final String nameField =
              (widget.component['headerNameField'] ?? '').toString();
          final String addrField =
              (widget.component['headerAddrField'] ?? '').toString();
          headerName =
              nameField.isNotEmpty ? (hDoc[nameField] ?? '').toString() : '';
          headerAddr =
              addrField.isNotEmpty ? (hDoc[addrField] ?? '').toString() : '';
        }
      }

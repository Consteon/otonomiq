...
class ChipAccent {
  ...
  static const ChipAccent blue = ChipAccent(Color(0xFFDBEAFE), Color(0xFF1D4ED8));
  static const ChipAccent red = ChipAccent(Color(0xFFFEE2E2), Color(0xFFDC2626));
  static const ChipAccent amber = ChipAccent(Color(0xFFFEF3C7), Color(0xFFB45309));
  static const ChipAccent green = ChipAccent(Color(0xFFDCFCE7), Color(0xFF16A34A));
  static const ChipAccent violet = ChipAccent(Color(0xFFEDE9FE), Color(0xFF7C3AED));
  static const ChipAccent teal = ChipAccent(Color(0xFFCCFBF1), Color(0xFF0D9488));
  static const ChipAccent slate = ChipAccent(Color(0xFFF1F5F9), Color(0xFF475569));

  /// Ordered palette for deterministic category assignment.
  static const List<ChipAccent> _palette = [
    blue, red, amber, green, violet, teal, slate,
  ];
...
CustomerOutstandingResult groupCustomerOutstanding({
  ...
  for (final Map<String, dynamic> doc in cacheDocs) {
    ...
    final Map<String, _Accum> custBucket =
        grouped.putIfAbsent(custId, () => <String, _Accum>{});
  ...
  for (final MapEntry<String, Map<String, _Accum>> custEntry
    ...
    for (final MapEntry<String, _Accum> itemEntry in itemMap.entries) {
      ...
      final int ageDays =
          acc.oldestT > 0 ? ((now - acc.oldestT) ~/ msPerDay) : 0;
      final int safeAgeDays = ageDays < 0 ? 0 : ageDays;

      final String resolvedName =
          itemNameMap[iid]?.isNotEmpty == true ? itemNameMap[iid]! : iid;
      final String resolvedCat = itemCatMap[iid] ?? '';

      items.add(CustomerItemSummary(
        itemId: iid,
    ...
    final String resolvedCustName =
        custNameMap[custId]?.isNotEmpty == true ? custNameMap[custId]! : custId;
    ...
    result.add(CustomerGroup(
      ...
      tier: agingTierDays(custOldestAgingDays,
          dangerDays: dangerAge, warnDays: warnAge),
      items: items,
    ));
...
class _CustomerOutstandingListState extends State<CustomerOutstandingList> {
  ...
  void _parseText() {
    ...
    try {
      _detailTextArray =
          diamondTextToList(widget.component['detailText'] ?? '');
    } catch (_) {
      _detailTextArray = [];
    }
  }
  ...
  void _subscribe() {
    ...
    final String rawCustTable =
        (widget.component['customerTable'] ?? '').toString().trim();
  ...
  (_IconEntry icon, ChipAccent accent) _resolveChip(
      String itemId, String itemCategory) {
    ...
    final _IconEntry fallbackIcon =
        const _IconEntry(emoji: '\u{25CF}', slot: ''); // ● neutral dot
                  ...
                  Flexible(
                    child: ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        final (icon, chipAccent) =
                            _resolveChip(item.itemId, item.itemCategory);
                        final (Color _, Color tierFg) =
                            AdminTierColors.outstandingBadge(
                          agingTierDays(item.agingDays,
                              dangerDays: _dangerAge, warnDays: _warnAge),
                        );
                        return Padding(
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: const Icon(Icons.search,
                    size: 18, color: AdminTierColors.mutedText),
                filled: true,
              ...
              style: const TextStyle(
                  fontSize: 13, color: AdminTierColors.titleText),
            ),
            const SizedBox(height: 12),

...
class _AdminActiveTripListState extends State<AdminActiveTripList> {
  ...
  void _parseConfig() {
    final String vnf =
        (widget.component['vehicleNameField'] ?? '').toString().trim();
    if (vnf.isNotEmpty) _vehicleNameField = vnf;
    final String ef =
        (widget.component['execField'] ?? '').toString().trim();
    if (ef.isNotEmpty) _execField = ef;
    final String nf =
        (widget.component['nameField'] ?? '').toString().trim();
    if (nf.isNotEmpty) _nameField = nf;
    final String iff =
        (widget.component['itemsField'] ?? '').toString().trim();
    if (iff.isNotEmpty) _itemsField = iff;
    final String dmf =
        (widget.component['doneMarker'] ?? '').toString().trim();
  ...
  Widget _buildCard(
    ...
    if (totalItems > 0) {
      subParts
          .add('$stopPrefix $totalDone $stopSep $totalItems');
    }

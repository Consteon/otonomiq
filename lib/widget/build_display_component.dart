...
Widget buildDisplayComponent(
  ...
  if (tip == 'display_images' || tip == 'displayimages') {
    ...
    try {
      ...
      if ((component['variant'] ?? '').toString().trim().toLowerCase() ==
        ...
        final int cols =
            ((component['columns'] as num?)?.toInt() ?? 4).clamp(1, 6);
        result = Container(
          margin: EdgeInsets.only(
            top: (component['beforeSpacing'] ?? 0.0).toDouble(),
            bottom: (component['afterSpacing'] ?? 0.0).toDouble(),
          ...
          child: LayoutBuilder(
            builder: (BuildContext context, BoxConstraints c) {
              ...
              return GridView.count(
                ...
                childAspectRatio:
                    cell > 0 ? cell / kQuickTileHeight : 1.0,
                shrinkWrap: true,
                physics: const NeverScrollableScrollPhysics(),
                padding: EdgeInsets.zero,
                children: buildGridList(
                    component['children'], fontSize, scrName,
                    variant: 'v6'),
              );
            },
          ),
    ...
    try {
      ...
      final List<String> labels =
          diamondTextToList((host['data'] ?? '').toString());
      ...
      for (final dynamic c in kids) {
        ...
        built.add(MapEntry(
          tabIndex,
          buildDisplayComponent(c, scrName, userRepository,
              dialog: dialog ?? false),
        ));
      }
      if (built.isEmpty) {
        // Visible, not a blank page: a bad `source` is otherwise silent.
        result = Text('--${component['type']}-- no sections'
        ...
        result = Container(
          ...
          child: TabSections(
            ...
            v6: (component['variant'] ?? '').toString().trim().toLowerCase() ==
                'v6',
    ...
    try {
      ...
      final List<String> blockText =
          diamondTextToList(component['text'] as String? ?? '');
      ...
      if (isV6) {
        for (int i = 0; i < childrenNum; i++) {
          ...
          component['children'][i]['blockTitle'] =
              blockText.isEmpty ? '' : blockText[0];
          component['children'][i]['row'] =
              attendRowShape(childrenNum) ? 'TRUE' : 'FALSE';
        }
      }
      for (int i = 0; i < childrenNum; i++) {
        late Widget theIcon;
            ...
            builder: (BuildContext context) => attendanceBlockV6(
              ...
              metaWidget: liveMeta
                  ? AttendMeta(
                      ...
                      component: component,
                      scrName: scrName,
                      prefix: blockText.length > 3 && blockText[3].trim().isNotEmpty
                          ? blockText[3]
                          : 'Terakhir',
                      yesterday:
                          blockText.length > 4 && blockText[4].trim().isNotEmpty
                              ? blockText[4]
                              : 'kemarin',
                      none: blockText.length > 5 && blockText[5].trim().isNotEmpty
                          ? blockText[5]
                          : 'Belum ada',
  ...
  } else if (tip == 'item_execution_submit' ||
      tip == 'itemexecutionsubmit') {
    try {
    ...
    try {
      ...
      if (component['valuePosition'] == null) {
        ...
        if (component['keyPosition'] != null) {
          txfControllerCheck(
              scrName, getPosition(component['keyPosition']));
        }

...
class _VehicleFeedListState extends State<VehicleFeedList> {
  ...
  void _subscribe() {
    ...
    final String rawOpeningGate =
        (widget.component['openingGate'] ?? '').toString().trim();
  ...
  void _onCardTap(VehicleFeedEntry entry) {
    ...
    transactionStore.dispatch(UpdateScreenTxAction(
        ScreenTransaction({'#ACTIVE_VEHICLE': entry.lv})));
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final List<VehicleFeedEntry> feed = buildVehicleFeed(
        ...
        executorNameField:
            (widget.component['executorNameField'] ?? 'dn').toString(),
      );
  ...
  Widget _buildSectionHeader(String label, List<VehicleFeedEntry> entries) {
    ...
    // others = textDim
    final bool isPerlu = entries.isNotEmpty &&
  ...
  Widget _buildVehicleCard({
    ...
    switch (entry.tier) {
      case VehicleTier.loading:
        ...
        cardBg = Colors.white;
        cardOpacity = 1.0;
        chip = const _ChipStyle('OPENING CHECK',
      ...
      case VehicleTier.custodyPending:
        ...
        cardOpacity = 1.0;
        chip = const _ChipStyle('CUSTODY PENDING',
      ...
      case VehicleTier.inRoute:
        ...
        cardBg = Colors.white;
        cardOpacity = 1.0;
        chip = const _ChipStyle('IN ROUTE',
      ...
      case VehicleTier.returning:
        ...
        cardBg = const Color(0xFFFFFBEB); // amber-50
        cardOpacity = 1.0;
        chip = const _ChipStyle('CLOSING CHECK',
            Color(0xFFFEF3C7), Color(0xFFD97706)); // amber
        final String time = formatEpochTime(entry.openingDoc?['ldt']);
        executorSuffix = time.isNotEmpty ? ' \u{00B7} tiba $time' : '';
    ...
    if (entry.tier == VehicleTier.inRoute) {
      ...
      // single assignment.
      summaryText = 'Mid-route \u{00B7} Stop ${entry.stopsDone}/${entry.stopsTotal}';
    } else if (entry.tier == VehicleTier.completed) {
      // Check for discrepancy in closing doc
      final String rs =
          (entry.closingDoc?['rs'] ?? '').toString().trim();
      if (rs == 'discrepancy_detected') {
        summaryText = 'Discrepancy detected';
      } else {
        summaryText =
            summaryText.isNotEmpty ? summaryText : 'Clean validation';
      }
    }

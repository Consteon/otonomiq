...
class _SignalListState extends State<SignalList> {
  ...
  String _txt(int specSegment) =>
      signalTextSegment(_textSegments, specSegment);
  ...
  void _initConfig() {
    ...
    if (_agingTimeField.isNotEmpty && _cadenceField.isEmpty) {
      devPrint('[signalList] AGING MODE with an EMPTY cadenceField — every '
    ...
    if (_statusEntries.isEmpty && _statusField.isNotEmpty) {
      devPrint(_agingTimeField.isEmpty
  ...
  List<Map<String, dynamic>> _serverFiltered() {
    ...
    final List<Map<String, dynamic>> sorted =
        List<Map<String, dynamic>>.from(augmented);
  ...
  List<Map<String, dynamic>> _augmentAndFilter(
    ...
    for (final Map<String, dynamic> doc in docs) {
      final int ds =
          computeSignalDaysSince(doc[_agingTimeField], nowMs);
      if (ds < 0) continue; // lo empty/non-numeric/future -> skip
      final num cadDays =
          computeSignalCadenceDays(doc[_cadenceField], _cadenceUnit);
      if (cadDays < 0) continue; // broken cadence -> skip (config-time log)
      final String st = computeSignalTier(
          ds, cadDays, _attentionFraction, _dormantMultiplier);
  ...
  List<Map<String, dynamic>> _applySearchBar(List<Map<String, dynamic>> docs) {
    ...
    return docs.where((Map<String, dynamic> d) {
      final String hay =
          '${_resolve(_txt(1), d)} ${_resolve(_txt(2), d)}'.toLowerCase();
  ...
  void _onCardTap(Map<String, dynamic> doc) {
    ...
    if (!routeExist(_route)) {
      devPrint('[signalList] route "$_route" is not in the page set — '
          'tap ignored');
      return;
    }
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          hintStyle: const TextStyle(
              fontSize: 13, color: AdminTierColors.mutedText),
        ...
        child: Row(
          children: <Widget>[
            const Icon(Icons.check_circle_outline,
                size: 20, color: AdminTierColors.okActionGreen),
  ...
  ListView _buildGrouped(List<Map<String, dynamic>> docs) {
    ...
    final List<Map<String, dynamic>> ungrouped = docs
        .where((Map<String, dynamic> d) =>
            (d[_groupField] ?? '').toString().isEmpty)
        .toList();
    ...
    return ListView(
      ...
      children: <Widget>[
        for (final String key in keys)
          if ((grouped[key] ?? const <Map<String, dynamic>>[]).isNotEmpty)
            ...<Widget>[
  ...
  Widget _buildCard(Map<String, dynamic> doc) {
    ...
    final SignalStatusEntry? status =
        lookupSignalStatus(_statusEntries, statusValue);
    ...
    final List<num> gaps =
        _gapsField.isEmpty ? const <num>[] : parseSignalGaps(doc[_gapsField]);
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      if (showTimeline) ...<Widget>[
                        const SizedBox(height: 10),
                        _buildTimeline(gaps, rawMarker, c,
  ...
  Widget _buildTimeline(
    ...
    final List<num> shown =
        folded ? gaps.sublist(gaps.length - 6) : gaps;
    final num? markerNum = num.tryParse(marker);
    // Non-numeric marker renders verbatim rather than lying with "0h".
    final String markerLabel =
        markerNum == null ? marker : '${formatSignalGapValue(markerNum)}h';
  ...
  Widget _gapDot() => Container(
        ...
        decoration: BoxDecoration(
          ...
          shape: BoxShape.circle,
          border:
              Border.all(color: AdminTierColors.mutedText, width: 1.5),
        ),
      );

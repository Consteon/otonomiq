...
class _CustodyRevealState extends State<CustodyReveal> {
  ...
  void _seedEditStore(List<Map<String, dynamic>> ipEntries) {
    ...
    final Map<String, CountEntry> editMap =
        CustodyReveal.getEditMap(widget.scrName);
  ...
  List<Map<String, dynamic>> _extractArray(
      Map<String, dynamic>? doc, String fieldName) {
  ...
  _CompareResult _compare(
    ...
    for (final key in allKeys) {
      ...
      final String category =
          (detail != null && detail.category.isNotEmpty)
              ? detail.category
              : '';
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      CustodyReveal.editRev.value; // revision signal dep: rebuild on clearEditState
      ...
      final String expectedField =
          (widget.component['expectedField'] ?? 'ie').toString().trim();
      final String actualField =
          (widget.component['actualField'] ?? 'ip').toString().trim();
      final List<Map<String, dynamic>> ieEntries =
          _extractArray(checkDoc, expectedField);
      final List<Map<String, dynamic>> ipEntries =
          _extractArray(checkDoc, actualField);
      ...
      final String categoryField =
          (widget.component['categoryField'] ?? 'ic').toString().trim();
      ...
      final _CompareResult result =
          _compare(ieEntries, editMap, itemDetailMap);
      ...
      final List<_CompareItem> returnableItems =
          result.items.where((i) => i.category == 'returnable').toList();
      final List<_CompareItem> consumableItems =
          result.items.where((i) => i.category == 'consumable').toList();
      ...
      final String mismatchLabel =
          _t(5, 'Ada selisih dengan catatan warehouse');
      final String matchBtnLabel =
          _t(6, 'KONFIRMASI LOAD \u{00B7} SIAP BERANGKAT');
      final String mismatchBtnLabel =
          _t(7, 'LANJUT \u{00B7} REPORT MISMATCH');
      ...
      final String recountRoute =
          (widget.component['recountRoute'] ?? '').toString().trim();
      ...
      final String reconcileField =
          (widget.component['reconcileField'] ?? 'rs').toString().trim();
          ...
          children: [
            ...
            if (returnableItems.isNotEmpty) ...[
              ...
              for (int i = 0; i < returnableItems.length; i++) ...[
                ...
                _buildEditableItemCard(
                  returnableItems[i], matchLabel, warehouseLabel, palette),
              ],
            ...
            if (consumableItems.isNotEmpty) ...[
              ...
              for (int i = 0; i < consumableItems.length; i++) ...[
                if (i > 0) const SizedBox(height: 12),
                _buildEditableItemCard(
                  consumableItems[i], matchLabel, warehouseLabel, palette),
              ],
              const SizedBox(height: 20),
            ],
            ...
            if (ungroupedItems.isNotEmpty) ...[
              for (int i = 0; i < ungroupedItems.length; i++) ...[
                if (i > 0) const SizedBox(height: 12),
                _buildEditableItemCard(
                  ungroupedItems[i], matchLabel, warehouseLabel, palette),
              ],
              const SizedBox(height: 20),
            ],
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: isMatch ? palette.greenBtn : palette.amberBtn,
    ...
    return Column(
      ...
      children: [
        ...
        CustodyStepper(
          value: item.actual,
          onDecrement: () {
            _onRevealCountChanged(
                countKey, item.actual - 1, item.ii, item.cd);
          },
          onIncrement: () {
            _onRevealCountChanged(
                countKey, item.actual + 1, item.ii, item.cd);

...
class _TaskCreateSuccessState extends State<TaskCreateSuccess> {
  ...
  Widget build(BuildContext context) {
    final Map<String, dynamic>? data =
        AdminCreateTaskSupport.getLastCreated(_wizardKey);
  ...
  Widget _buildSummaryCard(
      String kn, String vn, int totalDrop, int totalPickup) {
    return Container(
      width: double.infinity,
      decoration: BoxDecoration(
        color: Colors.white,
        ...
        child: ClipRRect(
          ...
          child: Row(
            ...
            children: [
              Container(
                constraints:
                    const BoxConstraints(maxWidth: 3, minWidth: 3),
                color: _infoBlue,
              ),
              Expanded(
                child: Padding(
  ...
  void _navigateToRoute(String fieldName) {
    final String rawRoute =
        (widget.component[fieldName] ?? '').toString().trim();

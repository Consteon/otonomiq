...
class _GroupPickerState extends State<GroupPicker> {
  ...
  String get _pairSep =>
      (widget.component['pairSep'] ?? '\u{25C6}').toString();
  ...
  String get _joinSep {
    ...
    return (raw != null && raw.toString().isNotEmpty) ? raw.toString() : _pairSep;
  }

  String get _itemSep =>
      (widget.component['itemSep'] ?? '\u{2B58}').toString();
  ...
  void _parseGroups() {
    ...
    for (final dynamic g in rawGroups) {
      ...
      final String src =
          (g['src'] ?? 'static').toString().trim().toLowerCase();

      if (src == 'table') {
        _groups.add(_GroupConfig(
          key: key,
          label: label,
          isTable: true,
        ...
        _groups.add(_GroupConfig(
          ...
          staticOptions: GroupPicker.parseStaticOptions(
              (g['options'] ?? '').toString(), pairSep, itemSep),
        ));
      }
    }
  }
  ...
  void _setActiveIndex(int idx) {
    ...
    GroupPicker._activeGroupStore
        .putIfAbsent(widget.scrName, () => {})[_valuePosition] = idx;
    _searchCtrl.clear();
    _query = '';
    _writeToController();
  }
  ...
  Set<String> _selectedIds(String groupKey) {
    return GroupPicker
            ._selectionStore[widget.scrName]?[_valuePosition]?[groupKey] ??
        {};
  }

  Map<String, String> _selectedLabels(String groupKey) {
    return GroupPicker
            ._labelStore[widget.scrName]?[_valuePosition]?[groupKey] ??
        {};
  }
  ...
  void _writeToController() {
    ...
    txfController[scrName]![vPos]!.finalData =
        GroupPicker.encodeSelection(idList, joinSep, isSingle: _isSingle);
    ...
    if (lPos != null) {
      ...
      final List<String> labelList =
          idList.map((v) => labels[v] ?? '').toList();
      txfController[scrName]![lPos]!.finalData =
          GroupPicker.encodeSelection(labelList, joinSep, isSingle: _isSingle);
    }
  }
  ...
  List<dynamic> _activeRows() {
    ...
    if (group.isDoc) {
      ...
      final List<Map<String, String>> options =
          GroupPicker.parseStaticOptions(fieldValue, _pairSep, _itemSep);
      ...
      if (_query.isNotEmpty) {
        return TablePicker.clientSearch(
            filtered, _query, group.labelField, group.subField);
      }
      return filtered;
    } else {
      if (_query.isNotEmpty) {
  ...
  String _rowId(dynamic row, _GroupConfig group) {
    if (group.isTable && !group.isDoc) {
      return TablePicker.resolveValueFromDoc(
          row as Map<String, dynamic>, group.valueField);
    }
        ...
        child: Column(
          ...
          children: [
            ...
            if (_hint.isNotEmpty && selected.isEmpty)
              Padding(
                ...
                child: Text(
                  _hint,
                  style: const TextStyle(
                      fontSize: 12, color: Color(0xFF9CA3AF)), // gray-400
                ),
              ),
            // Segmented selector
            ...
            if (rows.isEmpty)
              ...
              ConstrainedBox(
                constraints:
                    BoxConstraints(maxHeight: _maxListHeight(context)),
                    ...
                    children: [
                      for (final dynamic row in rows) ...[
                        _itemTile(
                          ...
                          isSelected:
                              selected.contains(_rowId(row, activeGroup)),
  ...
  Widget _selectAllTile(
    ...
    final int selectedCount =
        visibleIds.where((id) => selected.contains(id)).length;
            ...
            Icon(
              _isSingle
                  ...
                  : (isSelected
                      ...
                      : Icons.check_box_outline_blank),
              size: 20,
              color: isSelected ? primary : const Color(0xFF94A3B8), // slate-400
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(

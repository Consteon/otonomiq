                    ...
                    Wrap(
                      ...
                      children: [
                        if (totals.totalDrop > 0)
                          _totalPill('${_t(1, 'Drop')} ${totals.totalDrop}',
                              const Color(0xFF3B82F6)), // blue-500
                        if (totals.totalPickup > 0)
                          _totalPill('${_t(2, 'Pickup')} ${totals.totalPickup}',
                              const Color(0xFF8B5CF6)), // violet-500
                        if (totals.totalSale > 0)
                          _totalPill('${_t(3, 'Jual')} ${totals.totalSale}',
                              const Color(0xFFF59E0B)), // amber-500
                        if (totals.totalPurchase > 0)
                          _totalPill('${_t(4, 'Beli')} ${totals.totalPurchase}',
                              const Color(0xFF8B5CF6)),
                        if (totals.totalRefill > 0)
                          _totalPill('${_t(5, 'Refill')} ${totals.totalRefill}',
  ...
  Widget _txChip(String tx) {
    ...
    return Container(
      ...
      child: Text(label, style: TextStyle(fontSize: 10,
          fontWeight: FontWeight.w700, color: bg, letterSpacing: 0.3)),
    );
  }

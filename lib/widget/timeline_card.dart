...
class _TimelineCardState extends State<TimelineCard> {
  ...
  List<Map<String, dynamic>> _filtered() {
    ...
    if (_rawSearch.isNotEmpty) {
      try {
        ...
        devPrint('[timelineCard] search FAILED CLOSED (empty list) for '
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final bool chipInline = chip.label.isNotEmpty &&
          groups.isNotEmpty &&
      ...
      final bool showHeader = title.isNotEmpty ||
          showMore ||
            ...
            children: <Widget>[
              if (showHeader)
                Padding(
                  ...
                  child: _header(
                      title, moreLabel, showMore, chipInline ? null : chip),
                ),
  ...
  /// separator instead.
  Widget _header(
      String title, String moreLabel, bool showMore, TimelineChip? chip) {
    return Row(
      children: <Widget>[
        Expanded(
          child: Text(
  ...
  Widget _rowTileV6(Map<String, dynamic> doc, bool isLast) {
    ...
    final String dotValue =
        _chipField.isEmpty ? '' : (doc[_chipField] ?? '').toString().trim();
  ...
  Widget _chipPillV6(TimelineChip chip) {
    final Color ink =
        Color.lerp(statusColor(chip.tier), const Color(0xFF000000), 0.25)!;
    return Container(
      margin: const EdgeInsets.only(left: 8),
      height: 24,

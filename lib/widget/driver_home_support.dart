...
void resolveAndPublishActiveTrip(String scrName, String checkCode) {
  ...
  final List<Map<String, dynamic>> docs =
      List<Map<String, dynamic>>.from(mapTableContent[checkCode] ?? const []);
          ...
          for (final String key in mapTableContent.keys) {
            ...
            final List<Map<String, dynamic>> checkDocs =
                List<Map<String, dynamic>>.from(
                    mapTableContent[key] ?? const []);
            final String? result =
                computeActiveTripDocId(checkDocs, vid);
...
bool hideZeroEnabled(dynamic component) {
  ...
  return (component['hideZero'] ?? '').toString().trim().toUpperCase() == 'TRUE';
}
...
List<ConfirmedRow> buildConfirmedRows(
  ...
  if (groupByItem) {
    ...
    for (final MapEntry<String, List<Map<String, dynamic>>> g
      ...
      final bool isConsumable = consumableCategory.isNotEmpty &&
...
Future<bool> createNativeDocAutoId({
  ...
  try {
    ...
    unawaited(
      ref.set(docMap).then((_) {
        devPrint('[createNativeDocAutoId] acked $path/${ref.id}');
      }).catchError((Object e) {
        errorReport('[createNativeDocAutoId] late-fail $path/${ref.id}: $e');
      }),
    );

...
class ApproverStickyBar {
  ...
  static void register({
    ...
    final int idx =
        pos == null ? -1 : list.indexWhere((c) => c.component['position'] == pos);
    if (idx >= 0) {
      list[idx] = newConfig;
    } else {
      list.add(newConfig);
    ...
    if (rawGateTable.isNotEmpty && !_gateSubscriptions.containsKey(gateSubKey)) {
...
class StickyBarRenderer extends StatelessWidget {
  ...
  Widget _buildApproval(StickyBarConfig config) {
    ...
    final String rawGateTable =
        (config.component['gateTable'] ?? '').toString().trim();
    if (rawGateTable.isNotEmpty) {
      try {
        ...
        if (currentRow.isEmpty) {
          devPrint('StickyBar slot gate: currentRow empty '
              '→ buttons hidden (pre-publish or direct nav)');
          return const SizedBox.shrink();
        }

        final String rawGateSearch =
            (config.component['gateSearch'] ?? '').toString().trim();
        final String rawRowSlot =
            (config.component['gateRowSlot'] ?? '').toString().trim();
        final String decodedRowSlot =
            autheniumDecode(rawRowSlot) ?? rawRowSlot;
        ...
        if (gsc.isEmpty ||
          ...
          devPrint('StickyBar slot gate: incomplete config — expected '
        ...
        if (pointerIdx != null &&
          ...
          devPrint('StickyBar slot gate: pointerIdx=$pointerIdx '
        ...
        if (levelIdx != null &&
          ...
          devPrint('StickyBar slot gate: levelIdx=$levelIdx '
              'out of range for row.length=${currentRow.length} '
              '→ buttons hidden');
          return const SizedBox.shrink();
        }
        ...
        if (gateCode.isEmpty) {
          devPrint('StickyBar slot gate: no gate subscription for '
        ...
        final List<Map<String, dynamic>> matchedGrants =
            filterDriverHomeDocs(rawGateDocs, rawGateSearch, config.scrName);

        if (matchedGrants.isEmpty) {
          devPrint('StickyBar slot gate: no grant docs match gateSearch '
        ...
        for (final grantDoc in matchedGrants) {
          final String rawSlotVal =
              (grantDoc[gsc.slotField] ?? '').toString().trim();
        ...
        if (!isVisibleBySlotGate(
            pointer, level, allConcrete, allWildcardLevels)) {
          devPrint('StickyBar slot gate: mismatch pointer="$pointer" '
              'level="$level" concrete=$allConcrete '
              'wildcardLevels=$allWildcardLevels → buttons hidden');
          return const SizedBox.shrink();
        }
        devPrint('StickyBar slot gate: PASS pointer="$pointer" '

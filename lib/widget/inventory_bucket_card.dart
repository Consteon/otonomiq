...
class _InventoryBucketCardState extends State<InventoryBucketCard> {
  ...
  Map<String, String> _buildCategoryMap() {
    ...
    // (1) explicit override
    final String rawMap =
        (widget.component['categoryMap'] ?? '').toString().trim();
  ...
  List<_TypeBucketRow> _computeInventory(
    ...
    final String catField =
        (widget.component['categoryField'] ?? 'cd').toString().trim();
    final String itemField =
        (widget.component['itemField'] ?? 'ii').toString().trim();
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final List<Map<String, dynamic>> filtered = rawSearch.isNotEmpty
          ...
          : docs;
      final List<_TypeBucketRow> inventory =
          _computeInventory(filtered, itemNameMap);
  ...
  Widget _buildTypeCell(_TypeBucketRow row) {
    ...
    final String primaryLabel =
        _buckets.isNotEmpty ? _buckets[0].label : 'isi';
    ...
    final String secondaryText =
        secondaryParts.isNotEmpty ? secondaryParts.join(' · ') : '';

...
class ItemExecutionSubmit extends StatefulWidget {
  ...
  static Future<bool> _doActualWrite({
    ...
    final Map<String, dynamic>? taskDoc =
        findActiveTaskDoc(component, scrName);
    if (taskDoc == null) {
    ...
    final List<Map<String, dynamic>> items =
        extractItemsFromDoc(taskDoc, cfg.itemsField);
    ...
    if (items.isNotEmpty) {
      final Map<String, ExecutionEntry> execMap =
          ItemExecutionList.getExecMap(scrName);
    ...
    if (!success) {
      devPrint('[_doActualWrite] writeNativeFields failed for scrName=$scrName');
    } else if (!internetConnected()) {
      // Silent hook path (no BuildContext): devPrint only, per design.
      devPrint('[_doActualWrite] offline: write queued locally for '
  ...
  static Future<bool> runActualWrite(String scrName) async {
    final dynamic component =
        ItemExecutionList.submitComponentByScr[scrName];
  ...
  Future<void> _onTap(BuildContext context) async {
    ...
    try {
      ...
      if (rawUER.isNotEmpty) {
        final String uerResolved =
            resolveDriverCurlyTokens(rawUER, widget.scrName);
      ...
      if (rawATE.isNotEmpty) {
        resolved['addToEvent'] =
            resolveDriverCurlyTokens(rawATE, widget.scrName);
      }
      ...
      locSensor.nowTime =
          DateTime.fromMillisecondsSinceEpoch(saveSendTimestamp);
      final String locString = getLocationString('', '', '', locSensor);

      saveSend(
          saveSendTimestamp, widget.scrName, resolved, locString, defaultVid());
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final Color textColor =
          enabled ? Colors.white : const Color(0xFF6B7280); // gray-500
...
List<Map<String, dynamic>> extractItemsFromDoc(
    Map<String, dynamic>? taskDoc, String itemsField) {
...
ItemExecutionSubmitConfig parseConfigFromComponent(dynamic component) {
  String f(String key, String def) =>
      (component[key] ?? def).toString().trim();
...
Map<String, dynamic>? findActiveTaskDoc(dynamic component, String scrName) {
  ...
  final String taskCode = '${resolveAppVid(component)}/${tp.tableDocId}/${tp.subColl}';
  final List<Map<String, dynamic>> docs =
      List<Map<String, dynamic>>.from(
    ...
    switch (txKind) {
      case 'deliver':
        ...
        if (entry != null) {
          ...
          item[cfg.actualDropField] = int.tryParse(
                  (item[cfg.planDropField] ?? '0').toString().trim()) ??
              0;
          item[cfg.actualPickupField] = int.tryParse(
      ...
      case 'refill':
        item[cfg.actualRefillField] = int.tryParse(
                (item[cfg.refillField] ?? '0').toString().trim()) ??
            0;
        break;
    }

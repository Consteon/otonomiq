...
class _ListCardState extends State<ListCard> {
  ...
  void _subscribe() {
    ...
    if (_rawStatusSearch.isNotEmpty) {
      ...
      if (rawStatusTable.isNotEmpty && stp.tableDocId.isNotEmpty) {
        ...
        devPrint('[listCard] statusSearch set but statusTable "$rawStatusTable" '
  ...
  List<Map<String, dynamic>> _getServerFiltered() {
    ...
    final List<Map<String, dynamic>> gated =
        _applySlotGate(conditioned, rawGateDocs);
  ...
  List<Map<String, dynamic>> _applySlotGate(
    ...
    if (!_gateIntended) {
      devPrint('ListCard slot gate: _gateIntended=false '
    ...
    if (_gateSlotConfig.isEmpty ||
      ...
      devPrint('ListCard slot gate: gateTable authored but gate incomplete '
    ...
    try {
      ...
      if (matchedGrants.isEmpty) {
        devPrint('ListCard slot gate: no grant docs match gateSearch '
      ...
      if (allConcrete.isEmpty && allWildcardLevels.isEmpty) {
        devPrint('ListCard slot gate: ${matchedGrants.length} grant doc(s) '
      ...
      devPrint('ListCard slot gate: grants=${matchedGrants.length} '
      ...
      devPrint('ListCard slot gate: ERROR vid=$vid '
  ...
  Future<void> _onCardTap(Map<String, dynamic> doc,
      [String? routeOverride]) async {
  ...
  Widget _buildCard(Map<String, dynamic> doc, {String? groupRoute}) {
    ...
    final bool statusOn = _rawStatusSearch.isNotEmpty &&

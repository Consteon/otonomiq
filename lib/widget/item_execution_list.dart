...
class _ItemExecutionListState extends State<ItemExecutionList> {
  ...
  void _subscribe() {
    ...
    final String effectiveCapTable =
        ItemExecutionList.resolveCapTable(widget.component);
    ...
    final String rawDropCapTable =
        (widget.component['dropCapTable'] ?? '').toString().trim();
    final String rawSaleCapTable =
        (widget.component['saleCapTable'] ?? '').toString().trim();
    if (rawDropCapTable.isNotEmpty &&
        rawSaleCapTable.isNotEmpty &&
        rawDropCapTable != rawSaleCapTable &&
        !ItemExecutionList._capTableMismatchLogged
  ...
  List<_ItemRow> _buildRows(
    ...
    final String rawSaleCapSearch =
        (widget.component['saleCapSearch'] ?? '').toString().trim();
    ...
    if (capModeActive) {
      ...
      if (capResolved) {
        ...
        if (rawSaleCapSearch.isNotEmpty) {
          final String resolvedSaleCapSearch =
              rawSaleCapSearch.replaceAll('{vehicleId}', taskVv);
          final List<Map<String, dynamic>> filteredSaleCapDocs =
              filterDriverHomeDocs(
                  allCapDocs, resolvedSaleCapSearch, widget.scrName);
    ...
    for (int i = 0; i < items.length; i++) {
      ...
      if (txKind == 'deliver') {
        ...
        final String saleItemIi =
            (item[saleCapFields.key] ?? '').toString().trim();
        final bool saleCapActive =
            rawSaleCapSearch.isNotEmpty && capModeActive;
  ...
  Widget _buildReadOnlyTxCard(
    ...
    if (isConsumableEditable) {
      if (ItemExecutionList.atCap(consumableActual, saleCap)) {
        final String rawCapLabel =
            ItemExecutionList.resolveCapFields(widget.component).label;
        consumableStatus = _CellStatus(
            'capped', rawCapLabel.replaceAll('<max>', '$saleCap'));
      } else {
            ...
            _buildStepperCell(
              ...
              onDecrement: consumableActual > 0
                  ? () => _onConsumableQtyChanged(
                      row.key, consumableActual - 1, saleCap)
                  : null,
              onIncrement:
                  ItemExecutionList.atCap(consumableActual, saleCap)
                      ? null
                      : () => _onConsumableQtyChanged(
                          row.key, consumableActual + 1, saleCap),
            ),
...
class _ItemRow {
  ...
  final int? saleCap; // resolved sale cap (null = no cap / unresolved / non-sale)

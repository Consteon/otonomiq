...
class DigitPadState extends State<DigitPad> {
  ...
  void initState() {
    ...
    final int? serialSlot = digitPadParsePosition(
      _spec.str('serialSourcePosition'),
    );
    _serialSourcePosition = (serialSlot != null && serialSlot == _position)
        ? null
        : serialSlot;
    ...
    try {
      ...
      if (positions.isEmpty) {
        if (block && !_gateVoidLogged) {
          ...
          devPrint(
            'DigitPad gate: wanted to block "${widget.scrName}" but no '
  ...
  Widget _content(
    ...
    if (_serialField.isNotEmpty &&
      ...
      devPrint(
        'DigitPad serial: "${widget.scrName}" has no component at '
        'serialSourcePosition $_serialSourcePosition — check OFF',
      );
    }
    final bool serialOn =
        _serialField.isNotEmpty &&
        _serialSourcePosition != null &&
        serialIc != null;
    ...
    final String serialReadRaw = serialOn
        ? _slotValue(_serialSourcePosition)
        : '';
    ...
    final bool serialProblem =
        serialState == DigitPadSerialState.mismatch ||

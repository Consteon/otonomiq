...
class DigitPadState extends State<DigitPad> {
  ...
  void initState() {
    ...
    _position =
        digitPadParsePosition((widget.component['position'] ?? '').toString());
    ...
    _digitsSourcePosition =
        digitPadParsePosition(_spec.str('digitsSourcePosition'));
    ...
    _blackOptions =
        digitPadParseOptions(_spec.list('digitsOptions'), allowZero: false);
    // allowZero: a '0' entry in digitsRedOptions is the MEANINGFUL choice
    // "no red digits" (Kawasan Ruko), not an empty value.
    _redOptions =
        digitPadParseOptions(_spec.list('digitsRedOptions'), allowZero: true);
  ...
  void _writeSlot(int? position, String value) {
    ...
    if (ic != null &&
        ic.finalData == value &&
        ic.controller.text == value) {
      return;
    }
    ocrWriteToPosition(widget.scrName, position,
        display: value, finalData: value);
  }
  ...
  Widget _content(
      EdgeInsets pad, int position, List<Map<String, dynamic>> docs) {
    ...
    final int? blackFromSlot =
        digitPadResolveCount(slotValue: blackSlot, docValue: null);
    final int? black = blackFromSlot ??
    ...
    final DigitPadLayout layout =
        black == null ? const DigitPadLayout(0, 0) : digitPadLayout(black, red);
    ...
    if (black == null && picker != DigitPadPickerState.forced) {
      ...
      return Padding(
        padding: pad,
        child: Text('--${widget.component['type']}-- Error: no digit count '
    ...
    _wantSource = hasBoxes
        ? digitPadResolveSource(_slotValue(_digitsSourcePosition),
            fromDoc: fromDoc)
        : '';
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            if (picker == DigitPadPickerState.forced || _pickerOpen) ...<Widget>[
            ...
            if (verdictText.isNotEmpty)
              _banner(
                ...
                verdictBg,
                verdictIcon,
                onTap: (verdict == DigitPadVerdict.spike ||
            ...
            if (verdictBlock && footer.isNotEmpty) ...<Widget>[
              ...
              Text(
                ...
                style: const TextStyle(
                    fontSize: 12.5, color: _backFg, fontWeight: FontWeight.w600),
              ),
            ],
          ],
        ),
  ...
  Widget _boxes(
      String buffer, bool enabled, int position, DigitPadLayout layout) {
          ...
          kids.add(GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: (enabled && i == caret) ? _caret : Colors.transparent,
                  width: 2,
                ),
              ),
              child: Text(
  ...
  Widget _picker(int position, bool enabled) {
    ...
    final int? currentBlack =
        digitPadResolveCount(slotValue: _slotValue(_digitsPosition), docValue: null);
        ...
        children: <Widget>[
          if (warn.isNotEmpty) ...<Widget>[
            Text(
              warn,
              style: const TextStyle(
                  fontSize: 12.5, color: _warnFg, fontWeight: FontWeight.w600),
            ),
            const SizedBox(height: 10),
          ],
  ...
  Widget _numpad(String buffer, bool enabled, int position) {
    ...
    Widget padKey(String id, String label, VoidCallback? onTap,
            {IconData? icon}) =>
        Expanded(
          child: Padding(
      ...
      children: <Widget>[
        ...
        Row(children: <Widget>[
          ...
          padKey(
            ...
            enabled
                ? () =>
                    _apply(digitPadPressBackspace(buffer, _cursor), position)
                : null,
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: Text(
                      message,
                      style:
                          TextStyle(fontSize: 14, color: fg, height: 1.35),
                    ),
                  ),
                ],
              ),

...
class TablePicker extends StatefulWidget {
  ...
  static String resolveValueFromDoc(
      Map<String, dynamic> doc, String valueField) {
...
class _TablePickerState extends State<TablePicker> {
    ...
    if (txfController[scrName]?[pos] != null) {
      ...
      if (stored.isNotEmpty && stored != emptyString) {
        final List<String> values =
            TablePicker.decodeSelection(stored, isSingle: _isSingle);
    ...
    if (txfController[scrName]?[_labelPosition] != null) {
      ...
      if (storedLabels.isNotEmpty && storedLabels != emptyString) {
        final List<String> labels =
            TablePicker.decodeSelection(storedLabels, isSingle: _isSingle);
        ...
        if (labels.length == vids.length) {
          ...
          TablePicker._labelStore.putIfAbsent(scrName, () => {})[pos] = labelMap;
        }
      }
    }
  }
  ...
  void _writeToController() {
    ...
    final List<String> labelList =
        valueList.map((v) => labels[v] ?? '').toList();

    txfController[scrName]![pos]!.finalData =
        TablePicker.encodeSelection(valueList, isSingle: _isSingle);
    txfController[scrName]![lPos]!.finalData =
        TablePicker.encodeSelection(labelList, isSingle: _isSingle);
  }
  ...
  void _openSheet() {
    ...
    final Map<String, String> workingLabels =
        Map<String, String>.from(_labelMap);
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border:
                      Border.all(color: const Color(0xFFD1D5DB)), // gray-300
                ),
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Icon(Icons.close,
                size: 14, color: primary.withValues(alpha: 0.55)),
          ),
        ],
      ),
    );
    ...
    setState(() {
      if (_selected.contains(vid)) {
        ...
        if (!TablePicker.canAddMore(
            currentCount: _selected.length, max: widget.max)) {
          return;
        }
        _selected.add(vid);
        _labels[vid] = label;
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final String vid = TablePicker.resolveValueFromDoc(
                          doc, widget.valueField);
                      final String label =
                          (doc[widget.labelField] ?? '').toString().trim();
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      widget.countTemplate
                          .replaceAll('{n}', _selected.length.toString()),
                      style: const TextStyle(
                        fontSize: 13,
                        fontWeight: FontWeight.w600,

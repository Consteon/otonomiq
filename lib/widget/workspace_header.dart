...
class _WorkspaceHeaderState extends State<WorkspaceHeader> {
  ...
  static const Color _accent = Color(0xFF4338CA); // indigo-700 (chip/address fg)
  ...
  int _deriveStopNumber(String activeTaskVid) {
    ...
    final String listSearch = (widget.component['listSearch'] ??
    ...
    final List<Map<String, dynamic>> ordered =
        filterDriverHomeDocs(allDocs, listSearch, widget.scrName);
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      final String taskId =
          (taskDoc?[idField] ?? '').toString().trim();
      final String customer =
          (taskDoc?[titleField] ?? '').toString().trim();
      final String address =
          (taskDoc?[addressField] ?? '').toString().trim();

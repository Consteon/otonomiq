...
VehicleSnapshot computeSnapshot(List<VehicleFeedEntry> entries) {
  ...
  for (final e in entries) {
    if (e.tier == VehicleTier.returning || e.tier == VehicleTier.custodyPending) {
      perluTindakan++;
...
String formatCategorySummary(Map<String, int> categoryCounts) {
  ...
  final List<String> others = categoryCounts.keys
...
List<VehicleFeedEntry> buildVehicleFeed({
  ...
  for (final slDoc in stockDocs) {
    ...
    final ({int done, int total}) progress =
        countStopProgress(lvTasks, tstField: tstField);

    entries.add(VehicleFeedEntry(
      lv: lv,
      plate: plate,
      driverVid: dv,
      driverName: dn,
...
List<({String label, List<VehicleFeedEntry> entries})> groupFeedBySections(
  ...
  final String l0 = sectionLabels.isNotEmpty ? sectionLabels[0] : 'Perlu Tindakan';
  final String l1 = sectionLabels.length > 1 ? sectionLabels[1] : 'Pengecekan Pembukaan';
  final String l2 = sectionLabels.length > 2 ? sectionLabels[2] : 'Dalam Perjalanan';
  final String l3 = sectionLabels.length > 3 ? sectionLabels[3] : 'Selesai Hari Ini';

  final List<({String label, List<VehicleFeedEntry> entries})> sections = [];
  if (perluTindakan.isNotEmpty) sections.add((label: l0, entries: perluTindakan));
  if (openingCheck.isNotEmpty) sections.add((label: l1, entries: openingCheck));
  if (dalamPerjalanan.isNotEmpty) sections.add((label: l2, entries: dalamPerjalanan));

...
AssetStockResult groupAssetStock({
  ...
  for (final Map<String, dynamic> doc in cacheDocs) {
    ...
    final Map<String, Map<String, int>> entityBucket =
        grouped.putIfAbsent(entityId, () => <String, Map<String, int>>{});
    final Map<String, int> pivotBucket =
        entityBucket.putIfAbsent(pivotVal, () => <String, int>{});
    ...
    if (detailField.isNotEmpty) {
      ...
      if (detailVal.isNotEmpty) {
        final Map<String, Map<String, Map<String, int>>> entityDetail =
            detailGrouped.putIfAbsent(entityId,
                () => <String, Map<String, Map<String, int>>>{});
        final Map<String, Map<String, int>> pivotDetail =
            entityDetail.putIfAbsent(pivotVal,
                () => <String, Map<String, int>>{});
        final Map<String, int> detailBucket =
            pivotDetail.putIfAbsent(detailVal, () => <String, int>{});
        detailBucket[condVal] = (detailBucket[condVal] ?? 0) + qty;

        // Name resolution: first non-empty wins
        if (detailNameField.isNotEmpty &&
            !detailNames.containsKey(detailVal)) {
          final String dName =
              (doc[detailNameField] ?? '').toString().trim();
          if (dName.isNotEmpty) {
        ...
        if (detailSubField.isNotEmpty &&
            !detailSubs.containsKey(detailVal)) {
          final String dSub =
              (doc[detailSubField] ?? '').toString().trim();
          if (dSub.isNotEmpty) {
  ...
  for (final MapEntry<String, Map<String, Map<String, int>>> entry
    ...
    final String resolvedName =
        itemNameMap[entityId]?.isNotEmpty == true ? itemNameMap[entityId]! : entityId;
...
class _AssetStockListState extends State<AssetStockList> {
  ...
  void _parseConfig() {
    ...
    _showCondition =
        cfg('showCondition', 'FALSE').toUpperCase() == 'TRUE';
    _filterTabs =
        cfg('filterTabs', 'TRUE').toUpperCase() != 'FALSE';
    _title = cfg('title', '');
    ...
    final String rawPivot =
        (widget.component['pivotValues'] ?? '').toString().trim();
    ...
    final String rawCond =
        (widget.component['condValues'] ?? '').toString().trim();
    ...
    final String rawSummary =
        (widget.component['summary'] ?? '').toString().trim();
    ...
    final String rawIconMap =
        (widget.component['itemIconMap'] ?? '').toString().trim();
      ...
      if (!rawJoinTable.contains('//') && _acCode.isNotEmpty) {
        ...
        if (mainTableDocId.isNotEmpty) {
          ...
          subscribeToMapCollection(
              appVid, mainTableDocId, rawJoinTable, _itemCode);
        }
      } else {
  ...
  (_IconEntry icon, ChipAccent accent) _resolveChip(
      String entityId, String entityCategory) {
    ...
    final _IconEntry fallbackIcon =
        const _IconEntry(emoji: '\u{25CF}', slot: ''); // bullet dot
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final List<String> pivotOrder =
          _pivotDefs.map((_PivotDef d) => d.value).toList();
      final List<String> condOrder =
          _condDefs.map((_CondDef d) => d.value).toList();
  ...
  int? _summaryTabIndex(String scope) {
    ...
    final String prefix =
        scope.contains('.') ? scope.substring(0, scope.indexOf('.')) : scope;
        ...
        for (int i = 0; i < _summaryDefs.length; i++) ...[
          ...
          Expanded(
            child: () {
              ...
              final bool isWarnScope = _condDefs.length > 1 &&
      ...
      child: Column(
        ...
        children: [
          ...
          if (activeTab == 0)
            ...
            _buildSinglePivotBody(
                entity, activeTab - 1, outstandingSublabel),
        ],
      ),
    );
  }
              ...
              child: Row(
                children: [
                  for (final (int idx, PivotCell cell) in nonZeroPivots)
                    Expanded(
                      flex: cell.total,
                      child: Container(color: _pivotAccent(idx).fg.withValues(alpha: 0.7)),
                    ),
                ],
              ),
            ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (_detailField.isNotEmpty &&
            ...
            Container(
              height: 1,
              color: accent.fg.withValues(alpha: 0.15),
            ),
            const SizedBox(height: 8),
          ...
          else if (!isOutstanding && _showCondition && _condDefs.isNotEmpty) ...[

...
class _FtzBluetoothPrinterState extends State<FtzBluetoothPrinter> {
  ...
  void initState() {
    ...
    if (_variant == 'share-pdf') {
      ...
      final String bwStr =
          (widget.component['borderWidth'] as String? ?? '8').toString().trim();
      _borderWidthPt = double.tryParse(bwStr) ?? 8.0;

      // Grid config: "KxB" (e.g. "4x4" = 4 cols x 4 rows).
      // Malformed -> WARN + fall back to single mode.
      final String? gridStr =
          (widget.component['grid'] as String?)?.trim().toLowerCase();
      if (gridStr != null && gridStr.isNotEmpty) {
        ...
        if (k != null && k >= 1 && b != null && b >= 1) {
          ...
          devPrint('[PRN share-pdf] WARN: malformed grid "$gridStr", '
  ...
  Future<void> _loadKeyedData() async {
    ...
    if (searchParts.length < 2) {
      if (!silent) _showSnackBar('PRN keyed: invalid search format', Colors.red);
      return;
    }
  ...
  Future<List<dynamic>> _queryNota(
    ...
    for (final fs.Source src in <fs.Source>[
      ...
      try {
        dynamic query =
            firestoreDb.collection(path).where(field, isEqualTo: value);
  ...
  Future<List<Map<String, dynamic>>> _loadKeyedDataGrid() async {
    ...
    final String resolvedValue =
        resolveDriverCurlyTokens(rawValue, _scrName);
    ...
    List<dynamic> docs =
        await _queryNota(path, searchField, resolvedValue, limit: null);
    if (docs.isEmpty) {
      final num? numValue = num.tryParse(resolvedValue);
      if (numValue != null) {
        docs =
            await _queryNota(path, searchField, numValue, limit: null);
      }
    }
  ...
  Future<void> _startSharePdfProcess() async {
    ...
    try {
      ...
      if (_isGridMode) {
        ...
        if (items.isEmpty) {
          final String notFoundLabel =
              _labels.length > 4 ? _labels[4] : 'Data not found.';
        ...
        resolvedName =
            resolveAndSanitizeFileName(_fileName, _tableData);
      }
      ...
      final String errorLabel =
          _labels.length > 3 ? _labels[3] : 'Failed to create PDF.';
  ...
  Widget _buildSharePdfContent(BuildContext context, bool isEnabled) {
    ...
    final String loadingLabel =
        _labels.length > 1 ? _labels[1] : 'Loading...';
    final String sharingLabel =
        _labels.length > 2 ? _labels[2] : 'Sharing...';
    ...
    final double? specificWidth =
        double.tryParse(widthValue?.toString() ?? '');

...
class _AdminOutstandingListState extends State<AdminOutstandingList>
  ...
  void _parseConfig() {
    final String tf =
        (widget.component['titleField'] ?? '').toString().trim();
    if (tf.isNotEmpty) _titleField = tf;
    final String itf =
        (widget.component['itemField'] ?? '').toString().trim();
    if (itf.isNotEmpty) _itemField = itf;
    final String qf =
        (widget.component['qtyField'] ?? '').toString().trim();
    if (qf.isNotEmpty) _qtyField = qf;
    final String af =
        (widget.component['ageAnchorField'] ?? '').toString().trim();
    if (af.isNotEmpty) _ageAnchorField = af;
    final String lnf =
        (widget.component['locationNameField'] ?? '').toString().trim();
    ...
    final String et =
        (widget.component['emptyText'] ?? '').toString().trim();
  ...
  void _subscribe() {
    ...
    final String rawSlTable =
        (widget.component['clientTable'] ?? '').toString().trim();
  ...
  Widget _buildRow(
      BuildContext context, OutstandingGroup group, String scheduleLabel) {
    // Tier badge colors

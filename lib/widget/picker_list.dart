...
class PickerList extends StatefulWidget {
  ...
  static int countForRow(
    ...
    final String resolved =
        resolveTimeTokens(decoded).replaceAll('{$idField}', rowId);
    ...
    if (resolved.contains('{') || rowId.isEmpty) {
      if (resolved.contains('{') && _loggedUnresolved.add(resolved)) {
        devPrint('[pickerList] countForRow: unresolved token in DSL '
...
class _PickerListState extends State<PickerList> {
  ...
  String _statusCode = ''; // status-table subscription code (separate from count)
  ...
  String get _rowIcon =>
      (widget.component['rowIcon'] ?? '').toString().trim();
  bool get _titleMono {
    final String raw =
        (widget.component['titleMono'] ?? '').toString().trim().toLowerCase();
    return raw == 'true' || raw == '1';
  }
  ...
  void _subscribe() {
    ...
    final String rawCountTable =
        (widget.component['countTable'] ?? '').toString().trim();
    ...
    if (_statusSearch.isNotEmpty) {
      final String rawStatusTable =
          (widget.component['statusTable'] ?? '').toString().trim();
      if (rawStatusTable.isNotEmpty) {
        ...
        if (stp.tableDocId.isNotEmpty) {
          ...
          subscribeToMapCollection(
              appVid, stp.tableDocId, stp.subColl, _statusCode);
        } else {
          devPrint('[pickerList] statusTable "$rawStatusTable" unparseable '
  ...
  void _select(String value, [Map<String, dynamic>? row]) {
    ...
    if (wizardKey.isNotEmpty) {
      final String vn =
          row != null ? (row[_titleField] ?? '').toString().trim() : '';
    ...
    if (_mode == 'navigate') {
      ...
      final String route =
          stripRouteWrapper((widget.component['route'] ?? '').toString().trim());
  ...
  Widget _rowTile(
    ...
    final String sub =
        _subField.isEmpty ? '' : (r[_subField] ?? '').toString().trim();
    ...
    if (_countCode.isNotEmpty) {
      final String rawCountSearch =
          (widget.component['countSearch'] ?? '').toString().trim();
      final int n = PickerList.countForRow(
          countDocs, rawCountSearch, _idField, id);
    ...
    if (_statusSearch.isNotEmpty &&
      ...
      final List<Map<String, dynamic>> pool =
          _statusCode.isNotEmpty ? statusDocs : countDocs;
      final int sn = PickerList.countForRow(
          pool, _statusSearch, _idField, id);
      statusIsOn = sn > 0;
  ...
  Widget _selectableRow({
    ...
    final Color primary = PickerList.parseHexColor(
          ...
          children: [
            ...
            Padding(
              ...
              child: Icon(
                ...
                size: 20,
                color: selected ? primary : const Color(0xFF94A3B8), // slate-400
              ),
            ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    if (!isBusy &&
                        hasStatusOrBadge(statusLabel, badge)) ...[
                      const SizedBox(height: 4),
                      Row(
                        children: [
                          if (statusLabel != null &&

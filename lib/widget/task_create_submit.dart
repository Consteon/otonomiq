...
class TaskCreateSubmit extends StatefulWidget {
  ...
  static bool resolveAdhocSkipTdt(dynamic component) {
    final String raw =
        (component['adhocSkipTdt'] ?? '').toString().trim().toLowerCase();
...
class _TaskCreateSubmitState extends State<TaskCreateSubmit> {
  ...
  Future<void> _onSubmit(BuildContext context) async {
    ...
    try {
      ...
      final String rawWhCfg =
          (widget.component['originWarehouse'] ?? '').toString().trim();
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final bool enabled =
          hasCustomer && (vehicleOptional ? hasItems : (hasItems && hasVehicle));

...
class _PayoutListState extends State<PayoutList> {
  ...
  List<Map<String, dynamic>> _getRows() {
    ...
    final String resolvedSearch =
        rawSearch.isNotEmpty ? _resolveSearch(rawSearch) : '';
    final List<Map<String, dynamic>> filtered =
        resolvedSearch.isNotEmpty ? PickerList.filterRows(docs, resolvedSearch) : docs;
  ...
  void _toggleSelectAll(List<Map<String, dynamic>> rows) {
    ...
    final List<String> visibleIds =
        rows.map((r) => _rowValue(r)).where((id) => id.isNotEmpty).toList();

    setState(() {
      if (visibleIds.isNotEmpty &&
          visibleIds.every((id) => ids.contains(id))) {
        // Deselect all.
  ...
  void _writeToController(List<Map<String, dynamic>> rows) {
    ...
    txfController[scrName]![pos]!.finalData =
        vals.isEmpty ? '' : vals.join(joinSep);
    ...
    if (lPos != null) {
      ...
      txfController[scrName]![lPos]!.finalData =
          vals.isEmpty ? '' : labels.join(joinSep);
    }
    ...
    if (tPos != null) {
      ...
      txfController[scrName]![tPos]!.finalData =
          (vals.isEmpty || !_moneyMode) ? '0' : total.toString();
    }
  }
  ...
  String _buildSummary(int workerCount, int grandTotal) {
    ...
    if (_moneyMode) {
      tpl = tpl.replaceAll('{total}', AdminCreateTaskSupport.formatRupiah(grandTotal));
    } else {
  ...
  String _buildSelectedCounter(int count, int selectedTotal) {
    ...
    if (_moneyMode) {
      tpl = tpl.replaceAll('{total}', AdminCreateTaskSupport.formatRupiah(selectedTotal));
    } else {
      tpl = PayoutList.collapseTemplate(tpl, '{total}', '');
    }
    return tpl;
  ...
  String _buildItemNominal(Map<String, dynamic> row) {
    ...
    tpl = tpl.replaceAll('{nom}', AdminCreateTaskSupport.formatRupiah(_rowNominal(row)));
    return tpl;
  }
  ...
  Widget _selectAllTile(
    ...
    final String label =
        _t(2, 'Pilih semua ({n})').replaceAll('{n}', rows.length.toString());

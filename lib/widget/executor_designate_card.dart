...
class _ExecutorDesignateCardState extends State<ExecutorDesignateCard> {
  ...
  void _subscribe() {
    ...
    final String rawBusyTable =
        (widget.component['busyTable'] ?? '').toString().trim();
    if (rawBusyTable.isNotEmpty) {
      ...
      if (bp.tableDocId.isNotEmpty) {
        ...
        subscribeToMapCollection(
            appVid, bp.tableDocId, bp.subColl, _busyCode);
      }
    }
  }
  ...
  void _showPicker(BuildContext context) {
    ...
    final String busyField =
        (widget.component['busyField'] ?? '').toString().trim();
    final String busyLabelField =
        (widget.component['busyLabelField'] ?? '').toString().trim();
    ...
    if (_busyCode.isNotEmpty && busyField.isNotEmpty) {
      ...
      busySet = ExecutorDesignateCard.buildBusySet(
          busyDocs, busyField, busyLabelField);
    } else {

...
TimelineChip inferChip({
  ...
  final TimelineChip empty =
      emptyLabel.trim().isEmpty ? nothingYet : TimelineChip(emptyLabel, 'muted');

  if (sortedDocs.isEmpty || chipField.isEmpty || chipMap.isEmpty) return empty;

  final Map<String, dynamic> top = sortedDocs.first;
  final int idx =
      chipIndexOf(chipMap, (top[chipField] ?? '').toString().trim());
...
TimelineChip? rowChip(
  ...
  final int idx = chipIndexOf(chipMap, (doc[chipField] ?? '').toString().trim());

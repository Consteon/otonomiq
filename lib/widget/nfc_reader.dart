...
class NfcReader extends StatefulWidget {
  ...
  static void clearCollectorState(String scrName) {
    collectorState
        .removeWhere((k, _) => k == scrName || k.startsWith('$scrName#'));
  }
...
class _NfcReaderState extends State<NfcReader> {
  ...
  void _doCollectorSuccess(String value, int groupIndex) {
    ...
    final String key =
        NfcReader.collectorKey(widget.scrName, groups[0].position);
    ...
    final String dedupe =
        (widget.component['dedupe'] ?? '').toString().toUpperCase();
    ...
    if (groupIndex < groups.length) {
      ...
      txfController[widget.scrName]![pos]!.finalData =
          joinIds(state[groupIndex]);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final String variant =
        (widget.component['variant'] ?? '').toString().toLowerCase();
  ...
  Widget _buildCollector(BuildContext context) {
    ...
    final String scanMode =
        (widget.component['scanMode'] ?? 'pad').toString().toLowerCase();
    final String emptyText = (widget.component['emptyText'] ?? '').toString();
    final String mismatchText =
        (widget.component['mismatchText'] ?? '').toString();
  ...
  Widget _buildStrip(
    ...
    return Column(
      ...
      children: [
        ...
        // Per-group sections
        for (int i = 0; i < groups.length; i++) ...[
          _buildGroupCard(groups[i], state[i],
              resolveTarget(groups[i].rawTarget, screenTx), emptyText),

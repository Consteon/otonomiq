...
class _VehicleCargoSummaryState extends State<VehicleCargoSummary> {
  ...
  void _subscribe() {
    ...
    final String rawCacheTable =
        (widget.component['cacheTable'] ?? '').toString().trim();
  ...
  List<Map<String, dynamic>> _getFilteredCacheDocs() {
    ...
    final String rawSearch =
        (widget.component['cacheSearch'] ?? '').toString().trim();
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final List<InlineSpan> introSpans =
          parseInlineEmphasis(introSrc, introBase);

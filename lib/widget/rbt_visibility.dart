...
RbtDocSource resolveRbtDocSource(String scrName) {
  try {
    final dynamic page =
        screenUIComponent is Map ? screenUIComponent[scrName] : null;
...
bool rbtChildVisible(
  ...
  for (final String clause in decoded.split(_kAndSep)) {
    ...
    if (key.isEmpty) {
      ...
      devPrint('RBT visibility: "$rawSearch" on "$scrName" — '
...
List<dynamic> visibleRbtChildren(List<dynamic> children, String scrName) {
  final List<dynamic> row = ItemCardDetail.currentRow.value;      // obs read #1 — literally first
  final RbtDocSource src = resolveRbtDocSource(scrName);
  final List<Map<String, dynamic>> docs =
      List<Map<String, dynamic>>.from(mapTableContent[src.code] ?? const []);   // obs read #2
  ...
  final List<Map<String, dynamic>> matched =
      filterDriverHomeDocs(docs, src.rawSearch, scrName);
  final Map<String, dynamic>? contextDoc =
      matched.isNotEmpty ? matched.first : null;

  return children.where((dynamic child) {
    final String raw =
        (child is Map ? (child['search'] ?? '') : '').toString().trim();

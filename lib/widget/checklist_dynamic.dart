...
const String checklistDefaultEmptyMessage =
    'Belum ada task untuk kategori ini.';
  ...
  for (int k = 0; k < block; k++) {
    if (k < titles.length) {
      ...
      out.add(
        checklistSerialize(
          titles[k],
          checklistLabelForKey(key, options, pendingLabel),
        ),
      );
    } else {
      out.add('');
...
class _ChecklistDynamicState extends State<ChecklistDynamic> {
  ...
  void _initConfig() {
    ...
    _labelToKey = checklistLabelToKey(<String>[
      for (final ChecklistOption o in _options) o.label,
    ], _pendingLabel);
    _emptyMessage = checklistEmptyMessage(_spec);
    _completedPrefix = checklistText(_spec, 1, checklistDefaultCompletedPrefix);
  ...
  void _apply(List<String> titles, String title, String newStatus) {
    final int block = checklistEffectiveBlock(
      _position,
      checklistBlockSize(_slots, titles.length),
    );
    if (block <= 0) return;
    final Map<String, String> statusByTitle = checklistStatusByTitle(
      _readBlock(block),
      _labelToKey,
    );
            ...
            children: <Widget>[
              GestureDetector(
                ...
                onTap: () => _apply(
                  titles,
                  title,
                  isDone ? checklistStatusPending : 'done',
                ),

...
class _CustodyEventSubmitState extends State<CustodyEventSubmit> {
  ...
  Future<void> _onTap(BuildContext context) async {
    ...
    try {
      ...
      final String cnmField =
          (widget.component['cnmField'] ?? 'cnm').toString();
  ...
  Widget build(BuildContext context) {
    ...
    if (_photoPos != null) {
      final String photoWidgetId =
          GeneralGetXController.to.getWidgetId(widget.scrName, _photoPos!);
  ...
  Widget _buildButton(BuildContext context) {
    ...
    final Color bgColor =
        enabled ? bgEnabled : const Color(0xFFD1D5DB); // gray-300

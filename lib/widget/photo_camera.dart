...
class PhotoCameraState extends State<PhotoCamera> with WidgetsBindingObserver {
  ...
  CameraController _newController(int cameraIndex) {
    ...
    c.addListener(() {
      ...
      if (c.value.hasError && !_errorReported) {
        _errorReported = true;
        errorReport('camera died after initialize: ${c.value.errorDescription}');
      }
      setState(() {});
    });
    return c;
          ...
          child: Column(
            ...
            children: <Widget>[
              Container(
                ...
                child: Icon(
                  denied ? Icons.videocam_off_outlined : Icons.no_photography_outlined,
                  size: 34,
  ...
  void _v6ToggleFlash() {
    ...
    if (!_isFront) {
      ...
      if (c != null && !identical(c, _disposedController)) {
        safeUnawaited(
            c.setFlashMode(_flashModeFor(flashIndex)), 'v6 selfie flash');
      }
    }
  }
  ...
  void _v6Flip() {
    ...
    // the wrong one.
    final int next = getCameraIndex(widget.cameras, _isFront ? 'back' : 'front');
    setState(() {
      selectedCamera = next;
      initializeCamera(selectedCamera, flashIndex);
    });
...
class _SelfieOvalPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _band(
      canvas,
      Rect.fromLTRB(0, 0, size.width, math.max(0, math.min(size.height, oval.top))),
      hold: barBottom,
      fromTop: true,
    );
    _band(
      canvas,
      Rect.fromLTRB(
          0, math.max(0, math.min(size.height, oval.bottom)), size.width, size.height),
  ...
  void _band(Canvas canvas, Rect band,
      {required double hold, required bool fromTop}) {

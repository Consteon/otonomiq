...
class _CustodyConfirmedListState extends State<CustodyConfirmedList> {
  ...
  void _subscribe() {
    ...
    if (rawFlowTable.isNotEmpty && _spec.str('flowSearch').isNotEmpty) {
      ...
      if (ftp.tableDocId.isNotEmpty) {
        ...
        subscribeToMapCollection(
            appVid, ftp.tableDocId, ftp.subColl, _flowCode);
      }
    }
  }
  ...
  Map<String, ItemCirculation> _buildFlowMap() {
    ...
    final List<Map<String, dynamic>> filtered =
        filterDriverHomeDocs(docs, flowSearch, widget.scrName);
    ...
    final List<Map<String, dynamic>> live =
        excludeByStatus(filtered, kDefaultExcludeStatus, statusField: 'tst');
  ...
  Widget build(BuildContext context) {
    ...
    final Color pillBg = HSLColor.fromColor(primary)
        .withLightness(0.92)
        .withSaturation(0.35)
        .toColor();
    final Color pillText = HSLColor.fromColor(primary)
        .withLightness(0.35)
        .withSaturation(0.8)
        .toColor();
    // Hint tints (same HSL pattern; all lightness/saturation in [0,1]).
    final Color hintBg = HSLColor.fromColor(primary)
        .withLightness(0.97)
        .withSaturation(0.35)
        .toColor();
    final Color hintBorder = HSLColor.fromColor(primary)
        .withLightness(0.90)
        .withSaturation(0.35)
        .toColor();
    final Color hintIconBg = HSLColor.fromColor(primary)
        .withLightness(0.90)
        .withSaturation(0.40)
        .toColor();
    final Color hintAccent = HSLColor.fromColor(primary)
        .withLightness(0.40)
        .withSaturation(0.70)
        .toColor();
    ...
    return Obx(() {
      ...
      final String actualField =
          (widget.component['actualField'] ?? 'ip').toString();
      ...
      final Map<String, ItemDetail> itemDetailMap =
          buildItemDetailMap(itemDocs);

      // ── Grouping config ──
      final bool grouped = sduiBool(_spec.str('groupByItem'));
      final Map<String, String> condLabels =
          parseCondLabels(_spec.str('condLabels'));
          ...
          children: [
            ...
            Container(
              decoration: BoxDecoration(
                ...
                border: Border.all(
                    color: const Color(0xFFF0F1F3), width: 1),
                boxShadow: [
                  BoxShadow(
              ...
              child: Column(
                children: [
                  for (int i = 0; i < rows.length; i++) ...[
                    _buildRow(rows[i], dropLabel, pickupLabel,
                        grouped: grouped),
                    if (i < rows.length - 1)
                      const Divider(
                        height: 1,
  ...
  Widget _buildRow(ConfirmedRow r, String dropLabel, String pickupLabel,
      {bool grouped = false}) {
    return Padding(
  ...
  Widget _buildHint(
      Color hintBg, Color hintBorder, Color hintIconBg, Color hintAccent) {
    return Container(
      decoration: BoxDecoration(
        color: hintBg,

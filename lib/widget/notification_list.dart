...
class _NotifRow {
  ...
  _NotifRow(this.vid, this.channel, this.pic, this.text, this.ms, this.route,
...
class _NotificationListState extends State<NotificationList> {
  ...
  void _onTapRow(_NotifRow r) {
    ...
    setState(() {
      ...
      if (!hasRoute) r.expanded = !r.expanded; // expand only when not navigating
    });
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<NotificationBloc, NotificationState>(
      ...
      builder: (context, state) {
        final channels =
            state is NotificationLoaded ? state.notifications : <Notification>[];
        ...
        final live =
            channels.where((c) => liveVids.contains(c.vid ?? '')).toList();
    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: ListView(
          ...
          children: <Widget>[
            _chip('Semua', activeVid.isEmpty, 0, primary,
                () => setState(() => _filterVid = '')),
            for (final c in channels)
              _chip(
  ...
  Widget _chip(String label, bool active, int unread, Color primary,
      VoidCallback onTap) {
    return Padding(
  ...
  void _deleteRow(_NotifRow r) {
    ...
    if (r.unread) {
      ...
      }, 'notificationList urd').catchError((_) {}); // offline: tx fails, next _loadAll heals it
    }
  }
}

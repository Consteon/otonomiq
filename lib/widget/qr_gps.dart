...
class QrGpsState extends State<QrGps> {
  ...
  Widget build(BuildContext context) {
    void saveData(String scrName, String locString) {
      ...
      timerBloc.add(const Start(duration: 5)); //  get timerBloc from transactionStore
...
//            "#ff3d00", "Batal", true, ScanMode.DEFAULT));
        if (barcodeScanRes.substring(1, 5) != "Fail") {
          ...
          OtqState locSensor =
              OtqState().getDataFrom(position.timestamp, position, placeMark);
          String locString = getLocationString(
              barcodeScanRes == empty ? '' : barcodeScanRes, '', '', locSensor);

...
final RegExp _ocrDatePattern = RegExp(
...
({int maxSize, int quality}) ocrImageParameter(SduiSpec spec) {
  ...
  try {
    final List<String> raw = spec
        .str('imageParameter', '400,400,80')
        .split(separator[4]);
    p = <int>[
...
String ocrTypeAt(List<String> types, int i) {
  final String v = types.length > i ? types[i].trim().toLowerCase() : '';
...
List<String> ocrTargetLabels(SduiSpec spec, List<int> targets) {
  return <String>[
    for (int i = 0; i < targets.length; i++)
      spec.text(5 + i).trim().isNotEmpty ? spec.text(5 + i) : '◁${targets[i]}▷',
  ];
}
...
String ocrRawSummary(List<OcrElement> els) {
  final String joined = els
      .map((OcrElement e) => e.text.trim())
...
void ocrWriteToPosition(
  ...
  try {
    ...
    final dynamic other = GeneralGetXController.to.getController(
      scrName,
      position,
    );

...
class CustodyDiscrepancyList extends StatefulWidget {
  ...
  @override
  State<CustodyDiscrepancyList> createState() =>
      _CustodyDiscrepancyListState();
}
...
class _CustodyDiscrepancyListState extends State<CustodyDiscrepancyList> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final String dpField =
          (widget.component['discrepancyField'] ?? 'dp').toString();
      final List<Map<String, dynamic>> dpEntries =
          _extractArray(checkDoc, dpField);
  ...
  Widget _buildCard(_DiscrepancyRow r) {
    ...
    final Color accent =
        isShort ? _kAmberStrong : (isOver ? _kVioletStrong : _kNeutral);
    final Color pillBg =
        isShort ? _kAmberPillBg : (isOver ? _kVioletPillBg : const Color(0xFFF3F4F6));
    final Color pillFg =
        isShort ? _kAmberPillFg : (isOver ? _kVioletPillFg : _kNeutral);
    final String pillLabel =
        isShort ? _t(4, 'Kurang') : (isOver ? _t(5, 'Lebih') : '—');

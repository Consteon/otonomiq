...
class _TimelineLedgerState extends State<TimelineLedger> {
  ...
  void _initConfig() {
    ...
    final int def =
        int.tryParse((c['periodDefault'] ?? '').toString()) ?? 0;
    ...
    final String expRaw = (c['expandable'] ?? '').toString().trim().toUpperCase();
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final String? condResolved =
          resolveConditionsFailClosed(condDecoded, _screenTx);
  ...
  Widget _buildSectionedList(
    ...
    for (final section in sections) {
      ...
      for (final group in section.groups) {
        items.add(_buildCard(group, nowMs, isGrouped,
  ...
  Widget _buildCard(LedgerGroup group, int nowMs, bool isGrouped,
      {String? expandKey}) {
    ...
    final String badgeLabel =
        badgeEntry != null ? badgeEntry.label : badgeValue;

    // Template resolution.
    final String headResolved =
        resolveMapTokens(_headText, repWithTs, const {});
    final String titleResolved =
        resolveMapTokens(_titleText, repWithTs, const {});
    final String subResolved = resolveMapTokens(
      _subText,
      repWithTs,
      {'n': '${group.docs.length}'},
    );

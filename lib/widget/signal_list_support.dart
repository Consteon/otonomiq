...
SignalStatusEntry? lookupSignalStatus(
    List<SignalStatusEntry> entries, String value) {
...
List<num> parseSignalGaps(dynamic raw) {
  ...
  for (final String part in s.split(sep)) {
    final num? n =
        num.tryParse(part.trim().replaceAll('"', '').replaceAll("'", ''));
    if (n != null) out.add(n);
  }
  return out;
}
...
String computeSignalTier(
    int ds, num cadDays, double attFraction, double dormantMult) {

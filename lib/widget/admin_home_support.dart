...
List<Signal> deriveAdminSignals({
  ...
  if (returnedGate.isNotEmpty) {
    for (final t in tasks) {
      ...
      final String reasonSuffix =
          reason.isNotEmpty ? ' \u{00B7} $reason' : '';
  ...
  if (noExecutorGate.isNotEmpty) {
    for (final entry in vehiclesByLv.entries) {
      ...
      final List<Map<String, dynamic>> assignedTasks = vehTasks
          .where((t) =>
              (t[statusField] ?? '').toString().trim() == 'assigned')
          .toList();
  ...
  if (blockedGate.isNotEmpty) {
    for (final vc in vehicleChecks) {
      ...
      final String ln =
          (vehiclesByLv[vv]?[plateField] ?? '').toString().trim();
...
Future<bool> executeUpdateEventRow({
  ...
  try {
    ...
    final int tableVid = int.tryParse(target.tablevid.trim()) ??
        int.tryParse(appVid) ??
        0;
    if (tableVid == 0) {
    ...
    if (docs.length > 1) {
      errorReport('[executeUpdateEventRow] ${docs.length} matches at $path; '
    ...
    devPrint('[executeUpdateEventRow] merged $patch into $path/${docs.first.id}');
    return true;
  } catch (e, st) {
...
(int done, int total) progressFromItems(
  ...
  final List<String> markers =
      doneMarker.split(',').map((m) => m.trim()).where((m) => m.isNotEmpty).toList();
...
List<OutstandingGroup> groupOutstanding({
  ...
  for (final ac in assetCacheDocs) {
    ...
    groups.putIfAbsent(lv, () => []).add(
          OutstandingItem(
              itemId: ii, qty: qty, agingDays: ageDays < 0 ? 0 : ageDays),
        );
  }
  ...
  for (final entry in groups.entries) {
    ...
    result.add(OutstandingGroup(
      clientId: lv,
      clientName: clientName,
      totalQty: totalQty,
      agingDays: maxAgingDays,
      tier: agingTierDays(maxAgingDays, dangerDays: dangerDays, warnDays: warnDays),

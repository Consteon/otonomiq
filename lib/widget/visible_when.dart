...
class VisibleWhenClause {
  ...
  @override
  String toString() => 'VisibleWhenClause($position $op '
...
List<VisibleWhenClause>? parseVisibleWhen(String raw) {
  ...
  for (final String part in decoded.split(kVisibleWhenAnd)) {
    ...
    if (operand.length >= 2 &&
      ...
      final int? ref =
          int.tryParse(operand.substring(1, operand.length - 1).trim());
      if (ref == null) return null; // ◁abc▷ -> fail open, not a literal
      out.add(VisibleWhenClause(
          position: position, op: op, literal: '', refPosition: ref));
      continue;
    }
...
bool visibleWhenVisible(
  ...
  if (clauses == null) {
    ...
    devPrint('visibleWhen: unparseable "$raw" on "$scrName" '
        '-> visible (fail-open)');
    return true;
  }

...
class _RouteFeedHeaderState extends State<RouteFeedHeader> {
  ...
  void _subscribe() {
    ...
    final String rawWorkforceTable =
    ...
    final String rawTaskTable =
        (widget.component['taskTable'] ?? '').toString().trim();
  ...
  Map<String, dynamic>? _findDriverDoc() {
    ...
    final List<Map<String, dynamic>> wfDocs =
        List<Map<String, dynamic>>.from(
            mapTableContent[_workforceCode] ?? const []);
    for (final doc in wfDocs) {
      final String docVid =
          (doc['VID'] ?? doc['vid'] ?? '').toString().trim();
  ...
  Map<String, dynamic>? _findVehicleDoc() {
    ...
    final String driverVid =
        (transactionStore.state.screenTx['#has_user_login'] ?? '').toString();
    if (driverVid.isEmpty) return null;
    final List<Map<String, dynamic>> slDocs =
        List<Map<String, dynamic>>.from(
  ...
  List<Map<String, dynamic>> _getFilteredTasks() {
    ...
    final String rawSearch =
        (widget.component['taskSearch'] ?? '').toString().trim();
    ...
    final String stateField =
        (widget.component['stateField'] ?? 'tst').toString();
    return excludeByStatus(filtered, kDefaultExcludeStatus,
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final String plateField =
          (widget.component['plateField'] ?? 'ln').toString();
      final String plate =
      ...
      final double progressFraction =
          total > 0 ? (completedCount + failedCount) / total : 0;

      // Stop count text: "{completed} / {total} stop"
      final String stopCountText =
          '$completedCount / $total $stopLabel';
      // Failed suffix: " · {failed} gagal" (amber) when failed > 0
      final String failedSuffix =
          failedCount > 0 ? ' \u{00B7} $failedCount $failedLabel' : '';

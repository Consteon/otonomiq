...
class CoordinationSignalList extends StatefulWidget {
  ...
  @override
  State<CoordinationSignalList> createState() =>
      _CoordinationSignalListState();
}
...
class _CoordinationSignalListState extends State<CoordinationSignalList> {
  ...
  void _parseFieldConfig() {
    ...
    _assignBusyLabel =
        (widget.component['assignBusyLabel'] ?? '').toString().trim();
    _assignBusySearch =
        (widget.component['assignBusySearch'] ?? '').toString().trim();
    _assignBusyTable =
        (widget.component['assignBusyTable'] ?? '').toString().trim();
    ...
    final String rawUG = (widget.component['unassignedGate'] ?? '').toString().trim();
    _unassignedGate = rawUG.isNotEmpty ? (autheniumDecode(rawUG) ?? rawUG) : '';
    final String rawRG = (widget.component['returnedGate'] ?? '').toString().trim();
    _returnedGate = rawRG.isNotEmpty ? (autheniumDecode(rawRG) ?? rawRG) : '';
    final String rawNG = (widget.component['noExecutorGate'] ?? '').toString().trim();
  ...
  void _subscribe() {
    ...
    // Stock_location
    final String rawSlTable =
        (widget.component['vehicleTable'] ?? '').toString().trim();
    ...
    // Evidence
    final String rawEvTable =
        (widget.component['evidenceTable'] ?? '').toString().trim();
    if (rawEvTable.isNotEmpty) {
      ...
      if (evp.tableDocId.isNotEmpty) {
        ...
        subscribeToMapCollection(
            appVid, evp.tableDocId, evp.subColl, _evCode);
      }
    }
    ...
    final String rawAcTable =
        (widget.component['assetTable'] ?? '').toString().trim();
    if (rawAcTable.isNotEmpty) {
      ...
      if (acp.tableDocId.isNotEmpty) {
        ...
        subscribeToMapCollection(
            appVid, acp.tableDocId, acp.subColl, _acCode);
      }
    }
  }
  ...
  void _launchVehiclePicker({
    required String mode,
    required String taskVid,
  }) {
    ...
    if (_assignBusySearch.isNotEmpty && busyDocs.isEmpty) {
      devPrint('[coordinationSignalList] busy-guard pool empty for '
  ...
  void _navigateCross(String vehicleId) {
    final String crossRoute =
        (widget.component['crossRoute'] ?? '').toString().trim();
    if (crossRoute.isEmpty) return;

    // Build routeParams DSL: "gudangVehicle◼{vehicleId}" with literal vehicleId
    final String rawDsl =
        (widget.component['crossRouteParams'] ?? '').toString().trim();
  ...
  String _clusterLabel(String type, int count) {
    switch (type) {
      case 'unassigned_vehicle':
        return _t(5, '{n} order menunggu kendaraan')
            .replaceAll('{n}', '$count');
      case 'task_returned':
        return _t(6, '{n} order dikembalikan driver')
            .replaceAll('{n}', '$count');
      case 'no_executor':
        return _t(7, '{n} kendaraan tanpa pengantar')
            .replaceAll('{n}', '$count');
      case 'blocked_departure':
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final List<Map<String, dynamic>> vehicleChecks =
          List<Map<String, dynamic>>.from(
              mapTableContent[_vcCode] ?? const []);
      final List<Map<String, dynamic>> evidenceDocs =
          List<Map<String, dynamic>>.from(
              mapTableContent[_evCode] ?? const []);
  ...
  Widget _buildSignalCard(BuildContext context, Signal signal) {
    ...
    final (Color buttonBg, Color buttonFg, bool isOutline) =
                ...
                child: Row(
                  children: [
                    const Icon(Icons.place_outlined,
                        size: 12, color: AdminTierColors.mutedText),

...
import 'global.dart';
import 'redux/screen_transaction.dart' show ScreenTransaction, UpdateScreenTxAction;
...
Future<void> bridgePushToInbox(String? ioPath, Map<String, dynamic> data) async {
  ...
  try {
    ...
    final msgCollRef = FirebaseFirestore.instance
        .collection('$ioPath/$threadVid/msg');
...
Future<void> _showLocalNotification(
  ...
  await flnp.show(
    DateTime.now().millisecondsSinceEpoch.remainder(1 << 31), // ms-res id, fits 32-bit
    title,
    body,
    details,
    payload: route ?? '',
...
class FirebaseNotifications {
  ...
  Future<void> setUpFirebase() async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      devPrint('[fcm] onMessage: data=${message.data}'
      ...
      final dp = (message.data['dp'] ?? message.notification?.body ?? '') as String;
      final nm = (message.data['nm'] ?? message.notification?.title ?? '') as String;
  ...
  Future<void> _registerToken() async {
    try {
      ...
      devPrint('[fcm] token: $token'); // logs null too, so a silent-null is visible
      if (token != null) {

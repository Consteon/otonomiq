...
class OcrSpikeApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      const MaterialApp(title: 'OCR spike', home: OcrSpikePage());
}
...
class _OcrSpikePageState extends State<OcrSpikePage> {
  ...
  Future<void> _pick() async {
    ...
    setState(() {
      _rows
        ..clear()
        ..addAll(
          picked.map((XFile f) => _Row(f.path, _expectedFromName(f.path))),
        );
      _summary = '';
    });
  }
  ...
  Future<void> _run() async {
    ...
    final TextRecognizer rec = TextRecognizer(
      script: TextRecognitionScript.latin,
    );
    try {
      for (final _Row r in _rows) {
        try {
          final RecognizedText out = await rec.processImage(
            InputImage.fromFilePath(r.path),
          );
    ...
    for (final _Row r in _rows) {
      b.writeln(
        '${r.hit ? "OK " : "MISS"} exp=${r.controller.text.trim()} '

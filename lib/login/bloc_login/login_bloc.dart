...
//  }
  ...
  Stream<LoginState> mapEventToState(LoginEvent event) async* {
    if (event is EmailChanged) {
      ...
      yield* _mapInvitationLoginFailedToState(
          event.inv, event.country, event.uid, event.status);
    ...
    try {
      switch (status) {
        ...
        case 4:
          ...
          break;
        case 7:
          yield LoginState.invExpired(state.signInMethod, state.loginUid,
              state.loginInv, state.loginCountry);
          break;
      }
    } catch (_) {
      yield LoginState.failure(state.currentState, state.signInMethod,
  ...
  Stream<LoginState> _mapSmsCodeChangedToState(String smsCode) async* {
    yield state.update(
      isSmsCodeValid: Validators.isValidSmsCode(smsCode),
    );
  }
    ...
    try {
      ...
      switch (loginResult) {
        case 0:
          ...
          if (write) {
            storage.write(
                key: 'sd2', value: ftzSecretOneSeed2R); //   put Account key in secure storage
          } // end if (write)
          break;
        case 1:
  ...
  Stream<LoginState> _mapSendSmsPressedToState(String phone) async* {
    try {
      ...
      yield LoginState.smsVerification(
        state.currentState,
      );
    } catch (_) {
      yield LoginState.failure(state.currentState, 4, state.loginVid,
          state.loginInv, state.loginCountry);
    }
  }
  ...
  Stream<LoginState> _mapWrongVidToState(vidOK) async* {
    yield LoginState.invIsWrong(
        state.currentState, state.loginUid, state.loginVid);
  }

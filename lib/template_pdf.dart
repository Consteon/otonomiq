...
class TemplatePdf {
      ...
      for (int r = 0; r < gridRows; r++) {
        ...
        for (int c = 0; c < gridCols; c++) {
          ...
          if (idx < pageItems.length) {
            ...
            final cardWidgets =
                processBlock(0, _lines.length - 1, pageItems[idx]);
  ...
  List<pw.Widget> processBlock(
      int start, int end, Map<String, dynamic> context) {
    ...
    while (i <= end) {
      ...
      if (commandMatch != null) {
        ...
        switch (command) {
          ...
          case 'FEED':
            final linesToFeed =
                int.tryParse(attributes['lines'] ?? '1') ?? 1;
          ...
          case 'IMAGE':
            ...
            if (img != null) {
              ...
              widgets.add(
                pw.Align(
                  alignment: align,
                  child: _buildImage(img, attributes),
                ),
              );
            }
          ...
          case 'TEXT':
            ...
            if (singleLineContentMatch != null) {
              ...
              final content =
                  _lines.sublist(i + 1, endBlockIndex).join(';');
          ...
          case 'ROW':
            ...
            if (singleLineRowMatch != null) {
              widgets.add(_buildRow(
                  singleLineRowMatch.group(1)!, context, attributes));
            } else if (endBlockIndex != -1) {
              widgets.add(_buildMultiLineRow(
                  i + 1, endBlockIndex - 1, context, attributes));
              i = endBlockIndex;
            }
            break;
          case 'LOOP':
            ...
            if (singleLineLoopMatch != null) {
              widgets.addAll(_processSingleLineLoop(
                  singleLineLoopMatch.group(1)!, attributes, context));
            } else if (endBlockIndex != -1) {
              widgets.addAll(_processMultiLineLoop(
  ...
  pw.Widget _buildRow(String content, Map<String, dynamic> context,
    ...
    return pw.Row(
      crossAxisAlignment:
          _getCrossAxisAlignment(rowAttributes['align']),
      children: cols,
    );
  }

  pw.Widget _buildMultiLineRow(int start, int end,
      ...
      for (final match in commandMatches) {
        ...
        switch (command) {
          ...
          case 'TEXT':
            if (innerContent != null) {
              widgets
                  .add(_buildText(interpolate(innerContent, itemContext), attrs));
            }
            break;
          case 'IMAGE':
  ...
  pw.Widget _buildImage(
      pw.MemoryImage img, Map<String, String> attributes) {
  ...
  pw.Widget _buildColChild(String rawContent, Map<String, String> colAttrs,
      Map<String, dynamic> context) {
    final imageTagMatch =
        RegExp(r'^<IMAGE\s+([^>]*?)\s*/?>$').firstMatch(rawContent.trim());
    ...
    return text.replaceAllMapped(RegExp(r'\{\{([^}]+)\}\}'), (match) {
      ...
      if (format && formatter != null) {
        ...
        if (defaultMatch != null) {
          return valueAsString.isEmpty
              ? defaultMatch.group(1)!
              : valueAsString;
        }
    ...
    try {
      for (final part in parts) {
        ...
        if (currentValue is Map) {
          ...
          currentValue =
              (userIndex < currentValue.length) ? currentValue[userIndex] : null;
        } else {
          currentValue = null;
          break;
        }
  ...
  String _formatIdr(String numberString, int decimalDigits) {
    try {
      ...
      final number =
          double.tryParse(numberString.replaceAll(',', '')) ?? 0.0;

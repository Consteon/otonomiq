...
Future<FirebaseFirestore> _autheniumDb() async {
  ...
  final Iterable<FirebaseApp> existing = Firebase.apps.where(
    (a) => a.name == _kAutheniumApp,
  );
...
Future<Scheme3Keyring> autheniumKeys({String tenantId = ''}) async {
  ...
  try {
    ...
    _subs[filter] = query.snapshots().listen(
      (snap) {
        ...
        try {
          ...
          if (parsed.isNotEmpty) {
            ...
            devPrint(
              'autheniumKeys ${filter.isEmpty ? '<all tenants>' : filter}'
              ': ${snap.docs.length} doc(s), '
              '${parsed.entries.map((e) => 'v${e.key}x${e.value.length}').join(' ')}',
            );
          } else {
            devPrint(
              'autheniumKeys $filter: empty snapshot ignored, keeping '

...
Future<ScannerMatch> scannerValidateQr(
    String tableRaw, String searchField, String scanValue,
    {int? tenantVid, bool needRow = false}) async {
  ...
  devPrint('scannerValidateQr path=$path field=$searchField '
  ...
  if (rows.isNotEmpty) {
    ...
    devPrint('scannerValidateQr string match n=${rows.length} '
  ...
  if (numValue != null) {
    final dynamic snap2 = await firestoreDb
        ...
        .limit(lim)
        .get();
    final List<Map<String, dynamic>> rows2 =
        _rowsFromDocs(snap2.docs as List);
    if (rows2.isNotEmpty) {
      final ScannerMatch m2 = scannerMatchFromRows(rows2, needRow: needRow);
      devPrint('scannerValidateQr num match n=${rows2.length} '
...
Future<ScannerMatch> scannerVidInWorkforce(
    String tableRaw, String searchField, String vid,
    {int? tenantVid, bool needRow = false}) async {
  ...
  devPrint('scannerVidInWorkforce tableCode=$tableCode '
  ...
  if (tableCode.isEmpty) {
    ...
    return scannerValidateQr(tableRaw, searchField, vid,
        tenantVid: tenantVid, needRow: needRow);
  }
  ...
  if (needRow) {
    ...
    return scannerValidateQr(tableRaw, searchField, vid,
        tenantVid: tenantVid, needRow: true);
  }

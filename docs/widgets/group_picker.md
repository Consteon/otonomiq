...
- **State:** `static` maps on `GroupPicker` keyed by `scrName` + `valuePosition`. Cleared by `clearState(scrName)` from `buildPage(clear:true)` (page construction/refresh) and by `clearAll()` from `clearData` (per-navigation, every screen — see Reset-on-revisit). Plain `Map`s, mutated during build; the reactive part is the separate `static RxInt resetRev`, bumped by both clears and read as the first statement in build's `Obx`.
...
- **Reset-on-revisit (R7):** leaving the page and coming back always shows the FIRST group with nothing ticked — no selection or active tab survives a navigation, and the reset is already done on the **first painted frame** (no visible flash of the old state). Navigation (`gotoRoute` → `reloadPage`) returns the **cached** `linkElement[scrName]` and never re-runs `buildPage`, so the reset has to come from `clearData` — which runs **post-frame**. `clearData` therefore calls `GroupPicker.clearAll()`, wiping *every* screen rather than only the one being entered: a page is cleared on the nav **away** from it, while it is off-screen, so its next visit starts empty. `clearAll` also bumps `resetRev` so a same-page `clearData` (saveSend after submit, or a reset button) repaints the picker you are currently looking at — necessary because `clearData`'s `WidgetUpdateController.update()` only reaches `GetBuilder`-subscribed widgets, which this is not.

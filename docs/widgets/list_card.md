...
| `limit` | Optional row cap, written as a **plain number**: `"limit":5`. Absent, `""`, `"--"`, `0`, a negative, or anything unparseable = **unlimited** (today's behavior — every existing LIST_CARD is unaffected). A quoted `"5"` and an unquoted `5.0` both work; a quoted `"5.0"` does not and falls back to unlimited. The cut lands **after the sort** (so `t`+`desc` gives the N newest) and after the search-bar text filter, and **before grouping** — in grouped mode `limit` is therefore a **GLOBAL** cap of N rows *in total* across all sections, not N per section, and a group with no rows inside the cap is not rendered at all. The header count and the stats strip are **not** capped: they keep reporting the original `search`/`conditions` total. |
...
| `moreRoute` | Destination route for the header **"see all"** button. Empty (default) = no button. Rendered only when `moreRoute` AND `text` segment 5 are both set — a label with no route would be a dead tap. Separate from `route` on purpose: a teaser list is usually `route:""` (non-tappable rows) while its header still links to the full page. |
...
| `text` | Static labels, `◆`-joined: `headerTitle◆headerSubtitle◆countLabel◆searchHint◆emptyText◆moreLabel`. Any empty segment hides its element. Segment 5 (`moreLabel`, e.g. `Lihat Semua`) is **append-only** — every config deployed before it stops at segment 4, so the header button simply does not render there. |
...
- **Header count** shows the total (`search`/`conditions` result), not the live search-bar subset — and it is *not* reduced by `limit` either. Same for the stats strip: both read the pre-cut list on purpose ("5 teratas dari 42").
- **Teaser + "Lihat Semua" is the intended pairing for `limit`.** Set `limit` to the teaser size, add `text` segment 5 for the button label, and point `moreRoute` at the full-list screen — which itself carries the same `table`/`search` with NO `limit`. The button navigates with `routeStack.push` then `gotoRoute`; it resolves no `routeParams` and runs no biometric gate, because it opens a page rather than a specific record. An unresolvable `moreRoute` is a silent no-op, never a crash.
- **`limit` also switches the widget to fit-content height.** An uncapped list keeps the house-convention fixed viewport (`screenHeight * 0.79`) with its own inner scroll — the only shape that works for a list of unknown length. A capped list has a known-small maximum, so it sizes to its rows instead: no fixed height (which would strand dead space under a 3-row teaser), `shrinkWrap: true`, and `NeverScrollableScrollPhysics` — the page already scrolls (`main_page.dart` wraps `pageElements` in a `SingleChildScrollView`), so a second scroll inside it is redundant. There is no separate key for this: "bounded" is exactly what `limit` already declares. Side effect: a capped list that renders empty shows a compact `emptyText` rather than one centred in a tall box.
- **`limit` cuts the display list, not the data.** Pipeline order: `search` → `conditions` → slot gate → sort → search bar → `limit` → group/flat. The parse (`parseLimit`) and the cut (`applyLimit`) are pure functions in `list_card_support.dart`, unit-tested in [test/list_card_limit_test.dart](../../test/list_card_limit_test.dart).
- **`route:""` makes rows non-tappable** — this is the built-in behavior, not a separate flag. An empty `route` leaves `_routeStr` empty, so `_buildCard` passes `onTap: null` to the `InkWell`: no ripple, no navigation, no error. There is no chevron to hide (the card renders no trailing arrow). In grouped mode a group missing from `groupRoutes` gets the same treatment via `resolveGroupRoute`'s `''`. Caveat: the check is `isNotEmpty`, so a sheet delivering the sentinel `--` instead of `""` still shows a ripple (the tap is then a silent no-op because `routeExist('--')` is false) — author `""`.

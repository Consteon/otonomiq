...
| OcrCapture | [ocr_capture.md](ocr_capture.md) | draft | Photo → on-device ML Kit OCR (`ocr_capture`); own position = photo URL, recognised values written to `ocrTargets`; variants `auto` / `tap`; optional serial compare against a reference Firestore doc (`compareField`) with an inline mismatch warning at photo time, reusing DIGIT_PAD's `digitPadSerialState` |
...
| DisplayList | [display_list.md](display_list.md) | draft | Bordered list frame; variants `widget` (children), `tablecard1` (plain ListTile cards), `tableCardInteractive` (search + cards + detail popup via FtzArraySearch), `tableCardInteractiveV6` (frameless v6 report list: range chips, day groups, flat rows, semantic condition chip). Optional `rowSplit` splits bare `<N>` lines into stacked header/value |
...
| PhotoCamera | [photo_camera.md](photo_camera.md) | done | Still camera behind `acquireCamera`; `variant:"v6"` = full-screen Vertika v6 selfie (oval frame, labelled shutter row, explicit review step) |
...
| FtzScannerScreen | [ftz_scanner_screen.md](ftz_scanner_screen.md) | done | Full-screen QR camera pushed via the real Navigator; `variant: "v6"` gives the Vertika v6 layout (corner viewfinder, scrim, labelled controls in a bottom sheet, permission empty state) |
...
| Widget | File | Status | Description |
|---|---|---|---|
| LocationDetector | [location_detector.md](location_detector.md) | done | GPS location status with inside/outside site detection via LQR reference points; `variant: "v4"` or `"v6"` gives the Vertika navy panel, `"v6"` also opts the map screen in |
| MapPage | [map_page.md](map_page.md) | done | Full-screen location map pushed via the real Navigator from LocationDetector's "Lihat Peta"; `variant: "v6"` gives the Vertika v6 screen (geofence ring + accuracy circle, status sheet with Ke batas/Akurasi/Radius, honest "Belum bisa dipastikan" band, location-off empty state) |
...
| Widget | File | Status | Description |
|---|---|---|---|
| TabSections | [tab_sections.md](tab_sections.md) | done | SDUI `TAB`: section groups laid out `inline` / behind a `link` / under a tab bar; `variant:"v6"` gates the look only |

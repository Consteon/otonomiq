...
Same as `lib/template_printer.dart`. Tags: `<TEXT align bold underline height color>`, `<QRCODE data align size/>`, `<IMAGE src|url|asset align width height/>`, `<ROW align><COL width bold align>`, `<LOOP source='x'>`, `<HR>`, `<FEED lines='n'>`. `{{field}}` with `|idr`/`|usd`/`|default:X`. `<CUT>` silently ignored. `<GROUP_BY>` and `<ACCUMULATE>` silently ignored (v1).
...
- **`<IMAGE src='https://…' … />`, `<IMAGE url='…' … />`, or `<IMAGE asset='key' … />`:** Renders an image. `asset` = Flutter bundle (`assets/images/{key}.png`, offline-safe). `src` = network fetch by the widget's loader (e.g. a Firebase Storage branding image); `url` is a legacy alias for `src`. **Precedence: `src` > `url` > `asset`** — the first non-empty attribute is the primary source. Width/height in pt optional (default: natural size, fit container). If the primary (network) source fails — offline / non-200 / timeout / undecodable bytes — and an `asset` is ALSO declared, the `asset` is tried as a fallback FIRST; only if that also fails is the image skipped silently (footer just missing — no crash). Network fetch has a **5 s timeout** so a stalled host does not block PDF generation. Attribute NAMES are case-sensitive even though the tag name is not: `SRC=` / `URL=` / `ASSET=` yield no image. Works inline inside `<COL>`. **Quote a `src`/`url`** (it contains `/ ? & =`) and it must not contain `;` (the template line separator). Network images are re-fetched per share; offline shares lose them — declare an `asset` fallback (or bundle the image) if offline branding is required.

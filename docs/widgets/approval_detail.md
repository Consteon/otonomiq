...
| `gateRowSlot` | `String` | yes (when gating ON) | `slotField◆pointerIndex◆levelIndex`. The `pointerIndex` and `levelIndex` are **plain integers** — the **same index base as an integer-keyed RBT `search` clause** (`evaluateRbtSearch`, `approver_sticky_bar.dart:19-33`, which already resolves `ItemCardDetail.currentRow` by plain integer index with an `idx >= row.length` guard — that is how incident bars author `search: "5◼pending"`; a **field-keyed** clause such as `st◼waiting` is a different dialect, matched against the page context doc rather than the row — see [ftz_row_of_button_2.md](ftz_row_of_button_2.md)). They are 0-based into the published row, NOT the `◁N▷`/`◀N▶` marker grammar. Example: `sc◆5◆7`. |

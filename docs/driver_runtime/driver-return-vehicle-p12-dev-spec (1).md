...
### 3. `CIRCULATION_SUMMARY` (REUSE widget 208 — flow Jual/Beli) — ADDED 2026-06-24
Aktivitas/flow hari ini (BUKAN stok): total Drop / Pickup / **Jual** (full keluar) / Tukar (swap) / **Beli** (empty masuk). Beda dari "Sisa di Kendaraan" yang stok fisik. Baca `task.it[]` per tx (`pd`/`pp`/`ps`/`pr`/`pb`), `excludeStatus:load_rejected`. **Identik config yang dipakai di CustodyNotification (1016)** — 0 widget baru.
- Catatan: idealnya angka = ACTUAL (`ad`/`ap`, CF-derived) bukan plan; sementara CF off → pakai plan (`pd/ps/pr/pb`), oke krn task udah done. Doctrine: ini display recap, validasi tetap domain gudang.
```json
